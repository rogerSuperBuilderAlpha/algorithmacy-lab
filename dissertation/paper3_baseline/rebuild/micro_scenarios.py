...
def main():
    ...
    with open(out, "w") as fh:
        ...
        fh.write("\n## Honest bounds\n\n"
                 ...
                 "- The conduit/partial/strict assignments are contestable as a class, not only at "
                 "named edges: a peer payment is coded PARTIAL (the processor authorizes jointly) and "
                 "could be read CONDUIT; a GitHub PR PARTIAL (direct discussion plus a merge gate) and "
                 "could be read HIGHER where the gate is multi-party; ordering through a waiter CONDUIT, "
                 "where a reader who treats the kitchen as a joint determiner would code it otherwise. A "
                 "reasonable analyst could re-code many and move their bands; each rationale is the "
                 "auditable coding decision.\n"
