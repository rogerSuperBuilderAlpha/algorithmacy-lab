...
The dissertation develops the construct and builds a formal model of the condition that demands it, then grades that model across organizations, in three papers ordered from the conceptual to the formal. Paper 1, an integrative review, establishes that the triadic coordination form is real, that the dyadic constructs cannot be extended to it, and specifies algorithmacy to the discipline of construct clarity. Paper 2 adopts integrated information theory's measure of irreducibility, Φ, as a formal model: a coordination form is modeled as triadic exactly when its application-layer structure does not factor across the worker–system–counterpart partition, a condition that is computable, that the paper checks on controls whose answers are known, and that it presents as a modeling choice — with the measure's standing critiques acknowledged — rather than as a claimed identity. Paper 3 asks how far the model can be made graded, computing Φ across the complete family of three-node forms, across a population of simulated organizations, and across named real cases, and testing the model's central prediction in a controlled agent-based experiment. The mediation regime orders as the construct predicts, with strictly mediated coordination the most demanding, and the experiment confirms it behaviorally; but the graded magnitude is dominated by the determination function and is not a reliable scale, so Paper 3 presents Φ as a structural classifier and an ordinal, regime-level indicator. Its central result is that the score responds to determination structure and not to technology: a human-mediated court is modeled at the same score as an algorithmic platform of the same structure. The dissertation tests the model's core prediction but does not claim a fully validated measure; that fuller validation is the empirical program it opens.
