# The three-paper dissertation: the standard structure, and where ours stands
...
## Part 1 — The standard structure

A three-paper dissertation is **not three papers**. It is three standalone papers **plus a framing
architecture** that binds them into one argument. The standard has four components.

### 1. The introductory / overview chapter (Chapter 1)
A framing chapter whose job is not to introduce the research the way a single study's introduction does,
but to **state the overarching research problem and explain the relation among the three papers**
(Nillesen, 2018, on the paper-based thesis introduction: its goal "is not to give a comprehensive
introduction… but to explain the relation between the different publications"). It establishes the
**conceptual and theoretical linkages** across the papers, motivates the single overarching question,
previews each paper, and states how they connect and what they jointly contribute (Matteson & Lincoln,
2021; Ngwenya, 2024, on Chapter 1's "symbiotic relationship" to the chapters that follow).

### 2. The three papers as standalone chapters
Each is **independently publishable**, with its own introduction, methods, and conclusion, and each makes
a **distinct, substantive contribution** (De Jong, Moser & Hall, 2005; Anderson et al., 2021, on the
manuscript-style macrostructure). The conventional progression is **conceptual → empirical**, and the
middle paper bridges the conceptual first paper and the empirical last.

### 3. The integrative discussion / conclusion chapter
A synthesizing final chapter — the component that most distinguishes a dissertation from a stack of papers.
It is **not a summary**; it is a **higher-level synthesis** of the three papers' combined contribution and
overall significance, with implications and a future-research agenda (Lewis et al., 2021, "Writing a
...
(Matteson & Lincoln, 2021); "a collection of unrelated published papers is not acceptable." Coherence is
itself an evaluation criterion, achieved through **alignment between the articles and the thesis** (Kubota
et al., 2021) and a **unified purpose threading through every chapter** (Ali et al., 2023). The standing
failure mode is Morse's (2005) **"feigning independence,"** splitting one project into three thin slices;
the opposite risk is **compounding repetition** when each paper re-introduces shared apparatus (Mason &
...
## Part 2 — This dissertation against the standard
...
## Part 3 — What to add to meet the standard

Two framing chapters, drafted from material we already have.

### A. Introduction / overview chapter (Chapter 1)
- **The overarching problem:** the variance puzzle (equivalently positioned platform participants get
  divergent outcomes through identical systems) as the surface of a structural change — coordination has
  become triadic.
...
- A decision on **reference handling** (one consolidated list vs per-chapter), per Bentley's format rules.
- A light **redundancy pass** so that, with the framing chapters in place, the construct and the instrument
