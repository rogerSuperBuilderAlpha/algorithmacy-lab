...
circulant TPM ⇒ π = µ ⇒ ψ = 0. Run `python -m foundations.psi_vs_phi.psi` to execute the controls.
"""
...
def _reduced_subchain(J, sub, n):
    ...
    from foundations.candidate_audit.measures import marginal_lagged
...
def psi_partitioned(tpm_sbn, n):
    ...
    from foundations.candidate_audit.measures import _bipartitions
    P = convert.state_by_node2state_by_state(np.asarray(tpm_sbn, float))
    pi = stationary_distribution(P)
    J = pi[:, None] * P
