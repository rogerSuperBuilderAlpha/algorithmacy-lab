...
https://github.com/rogerSuperBuilderAlpha/algorithmacy-lab (`psi_vs_phi/`).
...
https://github.com/rogerSuperBuilderAlpha/algorithmacy-lab (`psi_vs_phi/`). The study reproduces
end-to-end: `python -m foundations.psi_vs_phi.psi` (ψ controls), `python -m foundations.psi_vs_phi.run 15 <seed>` (dataset,
seeds 1–5), `python -m foundations.psi_vs_phi.exact_phi3 15 1` (IIT-3.0 comparator, n = 3),
`python -m foundations.psi_vs_phi.dissociation` (analytic cases), `python -m foundations.psi_vs_phi.analyze` (statistics), and
`python -m foundations.psi_vs_phi.figures` (figures), built on the exact IIT-4.0 Φ oracle in `proxy_audit/`. For a
...
framework reused here were built in the companion `algorithmacy-lab` study. We thank A. Kearney for
