...
## Reproduce
`python -m foundations.cbh_complexity.complexity` → `python -m foundations.cbh_complexity.run` →
`python -m foundations.cbh_complexity.dissociation` → `python -m foundations.cbh_complexity.analyze` →
`python -m foundations.cbh_complexity.figures`. Built on the exact IIT-4.0 Φ oracle in `proxy_audit/`.
