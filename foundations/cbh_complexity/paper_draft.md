...
https://github.com/rogerSuperBuilderAlpha/algorithmacy-lab (`cbh_complexity/`).
...
All code, data, and figures are at https://github.com/rogerSuperBuilderAlpha/algorithmacy-lab
(`cbh_complexity/`). Reproduce end to end: `python -m foundations.cbh_complexity.complexity` (instrument controls),
`python -m foundations.cbh_complexity.run` (the three experiments, including the bootstrap grain sweep),
`python -m foundations.cbh_complexity.dissociation` (matched-entropy systems), `python -m foundations.cbh_complexity.analyze`
(summary statistics), and `python -m foundations.cbh_complexity.figures` (Figure 1). The exact-$\Phi$ oracle is reused
