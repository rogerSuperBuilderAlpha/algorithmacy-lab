#!/usr/bin/env python
"""Q53 / H2 — OR-graded worker-side back-channel restores Phi=2.0.

Run:  python -m org_frontier.questions.q53_impl_phi_ceiling.probe_worker_or_ceiling
"""
...
def main():
    print("PROBE 166 (H2) — OR-graded worker-side back-channel restores Phi=2.0")
    print("=" * 64)
    ...
    for label, rules in matched_implication_panel():
        bc = apply_topology(rules, "worker_or")
        v = verdict(bc, LABELS)
    ...
    print(f"\n  matched implication + worker OR at Phi=2.0: {at_phi2}/8")
    if at_phi2 >= 1:
        decision = "confirmed"
    ...
    with open(os.path.join(RESULTS_DIR, "worker_or_ceiling.csv"), "w", newline="") as fh:
