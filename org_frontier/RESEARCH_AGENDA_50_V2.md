...
1. Do ternary parties (idle / engaged / overcommitted) change which party is pivotal, or does the
   ...
   **Instrument — NOT_TESTABLE.** Path (C): IIT-4.0 pin rejects ternary n=3 SBS; (A)/(B) rejected
   for #1. Binary two-condition control holds. See `studies/ternary_pivotality/`;
   `BEYOND_BINARY_ARC.md`.
