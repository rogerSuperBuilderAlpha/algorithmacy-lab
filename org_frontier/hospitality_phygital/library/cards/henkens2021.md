---
citekey: henkens2021
title: "The Smarter, the Better?! Customer Well-Being, Engagement, and Perceptions in Smart Service Systems"
authors: "Henkens, Bieke and Verleye, Katrien and Larivi\`ere, Bart"
year: 2021
doi: 10.1016/j.ijresmar.2020.09.006
venue: "International Journal of Research in Marketing"
cluster: phygital-service
status: rejected
role: governor
used_by_sections: [4, 6]
claims: [wellbeing-valence, industry-gains-and-transition]
read_depth: full-text
verified: verified
verified_date: 2026-08-08
source_basis: "Full text obtained 2026-08-08 via open access (18,703 words) and read; identity confirmed against title and author before use. Bibliographic details verified against the publisher record."
generated_run: 2026-08-11-p9
flags: []
rejected_reason: "Status corrected 2026-08-11. The card asserted cited. The source is in neither references.bib nor cited_keys.txt, so it left the manuscript at some point after the 8 August build without the card following. The displacement argument was not recorded at the time and cannot now be reconstructed; this note is the record that it is missing."
---

> **Status corrected 2026-08-11 (P9 reconciliation).** This card's frontmatter had gone out of step with the manuscript, which moved after the 8 August build while the library stood still. The frontmatter below is now true; the prose beneath it predates the correction and has not been rewritten.

## Summary

Finds the well-being effects of smart service contingent on the person receiving it, so one
touchpoint may sit at different poles for different guests.

## Key facts / verified quotes with loci

- The well-being outcomes are specific: self-efficacy and technology anxiety, not well-being in general [full-text]
- Smartness reaches well-being through engagement with the *smart product*; the total effect of smartness on engagement with the service provider is 'insignificantly different from zero' [full-text]
- 'technological anxiety is an important inhibitor of customer well-being - and thus a provoker of customer ill-being - in a smart service system context' [full-text]
- *International Journal of Research in Marketing* 38:2, pp. 425-447 [metadata]


## Critical notes & threats

- A finding the paper should probably use rather than merely cite: smartness moves well-being through the guest's engagement with the device, and not through engagement with the provider. That is close to the substitutive pole stated in another vocabulary.
- The governor on the well-being claim, and the audit confirmed nothing better exists for mixed or contingent effects.
- Also does work in section 6: guest heterogeneity defeats any single specification of a good touchpoint.

## Where it enters the manuscript

Sections 4 and 6.
