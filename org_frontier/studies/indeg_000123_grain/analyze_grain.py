"""Joint role-target grain on indeg (0,0,0,1,2,3).

Both t_targets and z_targets vary. Hypotheses fixed in hypotheses.md.

Run:  python org_frontier/studies/indeg_000123_grain/analyze_grain.py
"""
...
# (cycles, recip, t_targets, z_targets, n_want, expect_phi, expect_ncore)
SAMPLE_PLAN = [
    (((2,), 1), (1,), (2, 2, 3), 2, 12.0, 4),
    (((2,), 1), (2,), (1, 2, 3), 3, 6.0, 3),   # Φ=6 joint cell
    (((2,), 1), (2,), (1, 3, 3), 2, 12.0, 4),   # same tt, different zt
    (((2,), 1), (2,), (3, 3, 3), 2, 12.0, 4),
    (((3,), 0), (1,), (2, 3, 3), 2, 12.0, 4),
    (((3,), 0), (2,), (2, 3, 3), 2, 12.0, 4),
]
...
def enumerate_pool():
    ...
    for choices in itertools.product(
        *[[j for j in range(N) if j != i] for i in range(N)]
        ...
        tt_set.add(tt)
        zt_set.add(zt)
        by[(feats[1], feats[2], tt, zt)].append(om)
    return by, tt_set, zt_set
...
def main():
    print("INDEG (0,0,0,1,2,3) JOINT ROLE-TARGET GRAIN — t × z")
    print("=" * 80)
    print("  cited: indeg_002122_grain ROLE_TARGETS_LAW; "
          "indeg_001113_grain Z_TARGETS_LAW")
    ...
    pool, tt_set, zt_set = enumerate_pool()
    print("SUBTYPE CATALOG")
    print("-" * 80)
    print(f"  t_targets values: {sorted(tt_set)}")
    print(f"  z_targets values: {sorted(zt_set)}")
    for key in sorted(pool):
        print(
            f"  cyc={key[0]} recip={key[1]} tt={key[2]} zt={key[3]}  "
            f"n={len(pool[key])}"
        )
    print()
    ...
    print("UNIFORMITY BY JOINT SUBTYPE")
    print("-" * 80)
    ...
    h1 = all(ok(k) for k in results)
    # H2: under tt=(2,), both 6 and 12
    h2 = (
        ok(((2,), 1, (2,), (1, 2, 3)))
        and ok(((2,), 1, (2,), (1, 3, 3)))
        and abs(results[((2,), 1, (2,), (1, 2, 3))][2] - 6.0) < PHI_EPS
        and abs(results[((2,), 1, (2,), (1, 3, 3))][2] - 12.0) < PHI_EPS
    )
    # H3: z=(1,2,3)→6; all other tested z→12
    h3 = True
    for key, (phis, cores, exp_phi, exp_nc) in results.items():
        zt = key[3]
        if zt == (1, 2, 3):
            if not all(abs(p - 6.0) < PHI_EPS for p in phis):
                h3 = False
        else:
            if not all(abs(p - 12.0) < PHI_EPS for p in phis):
                h3 = False
    # H4: (2,) contains both 6 and 12
    h4 = (
        ok(((2,), 1, (2,), (1, 2, 3)))
        and ok(((2,), 1, (1,), (2, 2, 3)))
        and abs(results[((2,), 1, (2,), (1, 2, 3))][2] - 6.0) < PHI_EPS
        and abs(results[((2,), 1, (1,), (2, 2, 3))][2] - 12.0) < PHI_EPS
    )
    h5 = ctrl and h1 and h2 and h4 and len(tt_set) > 1 and len(zt_set) > 1

    if h1 and h2 and h3 and h4 and h5:
        verdict_word = "JOINT_TZ_LAW"
        reading = (
            "JOINT_TZ_LAW — on indeg (0,0,0,1,2,3): both t and z vary; "
            "(tt,zt)=((2,),(1,2,3))→Φ=6 else Φ=12; t alone insufficient; "
            "z-cut sufficient; cycle alone mixes; no irreducible multi-role mix"
        )
    elif h1 and not h3:
        verdict_word = "JOINT_ONLY"
        reading = "JOINT_ONLY — need full (tt,zt) pair; z alone fails"
    elif not h1:
        verdict_word = "MIX_PERSISTS"
        reading = "MIX_PERSISTS — joint grain does not purify"
    else:
        verdict_word = "PARTIAL"
        reading = "PARTIAL — multi-role grain incomplete"

    print("HYPOTHESIS TESTS")
    print("-" * 80)
    print(f"  t_targets values: {sorted(tt_set)}")
    print(f"  z_targets values: {sorted(zt_set)}")
    print(f"  H1 (joint subtypes Φ-pure / law):   "
          f"{'SUPPORTED' if h1 else 'REFUTED'}")
    print(f"  H2 (t_targets alone insufficient):  "
          f"{'SUPPORTED' if h2 else 'REFUTED'}")
    print(f"  H3 (z_targets cut sufficient):      "
          f"{'SUPPORTED' if h3 else 'REFUTED'}")
    print(f"  H4 (cycle alone insufficient):      "
          f"{'SUPPORTED' if h4 else 'REFUTED'}")
    print(f"  H5 (multi-role grain, no irr. mix): "
    ...
    with open(os.path.join(RESULTS, "census.csv"), "w", newline="") as fh:
        ...
        for r in rows:
            core = r.get("core", ())
            core_s = "|".join(core) if isinstance(core, tuple) else str(core)
            w.writerow({
                "family": r.get("family", ""),
                "name": r.get("name", ""),
                "omit": r.get("omit", ""),
