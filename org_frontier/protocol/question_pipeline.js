...
Write a module that adds the repo root to sys.path and imports from org_frontier.* and proxy_audit.*. FIRST assert an instrument control passes (a known form reproduces its known verdict); abort if it fails. Then run the test, print exact numbers, and write any CSV to ${dir}/results/. Run it with ~/iit-playground/venv-4.0/bin/python -m org_frontier.questions.q${id}_${slug}.probe_${t.slug} (filter the PyPhi banner with grep -v). Report the exact numbers, the verdict (confirmed/partial/refuted — report refutation honestly), the probe file path, and a one-line PROBES.md result row.`,
