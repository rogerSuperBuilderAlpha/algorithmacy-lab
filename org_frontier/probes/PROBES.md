...
|---|---|---|---|---|
| 140 | Q6/H1 | smoothness of Φ_MIP(p) under commit noise | **partial** | exp fit max\|resid\|=0.0079 (≤0.02 ✓), residual ratio 4.06x (>3x ✗), Δ²Φ ratio 3.68x (<5x ✓, no localized spike); Φ smooth 2.0→0; refutation conditions unmet but 3× residual confirmation gate fails; injected-corner control spikes 9.18x at p=0.25. `results/q6_smoothness_sweep.csv` |
| 141 | Q6/H2 | susceptibility dΦ/dp peaks at interior p | **REFUTED** | \|χ\| strictly monotone-decreasing, global max at endpoint p=0.00 (\|χ\|=10.06), falling to 1.11 at p=0.50; no interior peak; instrument control p=0 triadic Φ=2.0 MIP {W,SC}. `probe_q6_susceptibility.py` |
| 142 | Q6/H3 | verdict flips only at p=0.5, no interior p* | **CONFIRMED** | noisy conjunctive chain, 51-pt grid p=0.00..0.50, classify+PHI_EPS=1e-9; triadic at all 49 interior pts + p=0; dyadic only at p=0.5 (Phi_MIP 2.0->0.0); major complex {W,S,C} throughout. `results/q6_verdict_interior.csv` |
| 143 | Q6/H4 | verdict/Φ decoupling | **confirmed** | single triadic->dyadic flip in [0.49,0.50] (width 0.01, constant before/after); max \|ΔΦ\| interval = 0.1006 = 5.03% of the 2.0 fall (<25%); Φ(0)=2.000 -> Φ(0.5)=0.000 — step verdict on a smooth-ramping Φ (Niizato 2020). |
| 144 | Q6/H5 monotonicity | Φ_MIP strictly decreasing in commit noise, no interior max | **confirmed** | 51-pt sweep: 0 rises (δ_k>1e-6), 0 interior peaks, min δ=-0.1006/max δ=-0.0111 all negative; instrument p=0=2.0 triadic {W,SC}; #81 U-shape control flags 25 rises. |

## Reading across Q6 (probes 140–144)

Commit noise drives the verdict to step, but Φ itself slides. The smoothness probe (140) fits Φ_MIP(p) with an exponential to a max residual of 0.0079, and the second-difference ratio of 3.68x shows no localized spike, so a phase transition cannot be read off the curve. The same probe withholds a clean confirmation: the residual ratio runs 4.06x where the gate demands under 3x, and an injected-corner control spikes 9.18x at p=0.25, so the curve is smooth-ish, not smooth. The susceptibility probe (141) settles the shape. dΦ/dp has its global maximum at the p=0 endpoint (|χ|=10.06) and falls monotonically to 1.11 by p=0.5, with no interior peak, which is the opposite of a critical point. The monotonicity probe (144) confirms the same decline from the other direction: across 51 points there are zero rises and zero interior peaks, every step negative, while a U-shape control flags 25 rises and shows the test can see non-monotonicity when it is there. The discontinuity that remains lives in the classifier. The verdict probe (142) holds triadic at all 49 interior grid points and flips to dyadic only at p=0.5, and the decoupling probe (143) pins that flip to a window of width 0.01 carrying just 5.03% of the total 2.0→0 fall. The transition is a threshold crossing in the verdict laid over a Φ that decays smoothly, which is the verdict/Φ decoupling Niizato (2020) describes, not a critical phenomenon in the integrated information.

### Question Q7 — party_vs_mediator_noise

| # | Concept (Q7) | Hypothesis | Verdict | Result |
|---|---|---|---|---|
| 145 | Q7/H1 | party (W) vs mediator (S) flip-noise Φ(p) sweeps, triadic conjunctive chain | **CONFIRMED** (g_max>1e-3) | g_max=0.9237 @ p=0.01; Φ_W(0.1)=0.7632 vs Φ_S(0.1)=1.1887 (g=−0.4255); self-ctrl=0.0; instrument p=0=2.0 triadic {W,SC}. `probe_party_vs_mediator.py` |
| 146 | Q7/H2 | W-noise vs C-noise are the same curve | **CONFIRMED** | exact-overlay null on d(p)=Φ_W−Φ_C, +flip-point match, +H1-gap positive control: 0/51 points \|d\|>1e-6, max\|d\|=0.0, flip_W=flip_C=0.5; pos-control 49/51 \|W−S\|>TOL, max\|W−S\|=0.9237 @p=0.01; instrument PASS (triadic, Φ=2.0, MIP {W,SC}). `probe_wc_symmetry.py` |
| 147 | Q7/H4 | party-noise steeper slope & below across interior | **REFUTED** | m_W=102.44 > m_S=10.06 (party 10.2x steeper, slope half holds) but Φ_W>Φ_S at 17/49 interior p (first reversal p=0.33); ordering reverses → refuted. Instrument 2.0 triadic {W,SC}; H1 g_max=0.924; self-ctrl Δ=0. `probe_slope_asymmetry.py` / `results/q7_slope_asymmetry.csv` |
| 148 | Q7/H5 | joint-party non-additivity | **partial** | r_max=1.4558 at p=0.01 (>1e-3=True); Φ_WC(0.1)=1.374 vs Φ_pred(0.1)=0.291 vs Φ_W(0.1)=0.763 (joint ABOVE separable null and ABOVE single party); p_eff(WC)=0.5 vs p_eff(W)=0.5 (earlier>=1step=False — collapse NOT advanced); self-control r_max=0e+00; H2 reuse max\|Φ_W−Φ_C\|=0. |
| 149 | Q7/H3 | party verdict-collapse point vs the degenerate endpoint | **REFUTED** | both strict and effective collapse sit at the degenerate endpoint for both sites: p_v(W)=p_v(S)=0.5, p_eff(W)=p_eff(S)=0.5; W-noise stays triadic through p=0.49 (Φ_W=0.0146) and flips dyadic only at p=0.5. Party noise leaves the verdict on the endpoint where mediator noise also leaves it; the seat changes Φ (probe 145) but not the flip point. Instrument p=0=2.0 triadic {W,SC}. `probe_verdict_flip.py` / `results/q7_verdict_flip.csv` |

## Reading across Q7 (probes 145–149)

Noise injected at a party and noise injected at the mediator do not act alike, and the asymmetry is the result. The flip-noise sweep (145) finds a real gap between the two curves: at p=0.1 the mediator curve sits higher (Φ_S=1.19) than the party curve (Φ_W=0.76), and the maximum separation reaches 0.92 near p=0.01, with the self-control gap pinned at zero so the difference is not a sweep artifact. The two parties are interchangeable. The symmetry probe (146) overlays the worker and counterpart curves exactly — 0 of 51 points differ above 1e-6, both flip at p=0.5 — while the same instrument still resolves the worker/mediator gap to 0.92 as a positive control, so the worker=counterpart null holds without the test being blind. The seat that carries the noise is what matters, and W and C are the same seat. The slope probe (147) refutes the clean ordering. Party noise is far steeper near the deterministic corner (m_W=102.4 against m_S=10.1, a 10x ratio), so the "steeper" half of the hypothesis holds, but the curves cross: Φ_W exceeds Φ_S at 17 of 49 interior points with the first reversal at p=0.33, so the party curve does not stay below the mediator curve and the ordering claim fails. The joint-party probe (148) lands partial. Injecting noise at both parties at once is non-additive — Φ_WC=1.37 at p=0.1 runs above both the separable prediction (0.29) and the single-party curve (0.76), so the joint effect does not factor — yet the effective collapse point does not advance (p_eff(WC)=p_eff(W)=0.5), so the non-additivity raises the interior curve without bringing the verdict flip forward. The verdict-flip probe (149) confirms why: party noise holds the triad through the entire interior and flips to dyadic only at the degenerate p=0.5 endpoint, the same endpoint the mediator site flips at, so changing the noised seat reshapes Φ(p) without moving the collapse point. The picture across the five: which seat is noised is what separates the curves, the two parties occupy one symmetric seat against the mediator's other seat, and jointly noising the parties is non-additive in Φ but not in the verdict threshold.

### Question Q8 — parity_vs_conjunctive_noise

| # | Concept (Q8) | Hypothesis | Verdict | Result |
|---|---|---|---|---|
| 150 | Q8/H1 | parity-vs-conjunctive verdict-flip, 4×51-pt flip-noise sweeps | REFUTED | p_v(parity,3)=p_v(parity,4)=p_v(conj,3)=p_v(conj,4)=0.50; all forms Phi_MIP>0 at every interior p, dyadic only at endpoint; parity does not flip interior | results/q8_verdict_flip.csv |
| 151 | Q8 | H2 — parity Φ decays faster than conjunctive in flip fraction (normalized decay) | REFUTED | parity wins 0/49 interior pts at n=3 and n=4 (conj ≥ parity at all 49); L_par=2.466 vs L_conj=5.162 (n=3), 10.173 (n=4); parity Φ̂ n-independent (0.763 at p=0.10 both sizes), conj decays faster — opposite direction | probe_normalized_decay.py |
| 152 | Q8/H3 | flip-gap parity vs conjunctive widens with n | REFUTED | strict G_v(3)=G_v(4)=0 (both flip only at p=0.5); effective register governs: G_eff(3)=-0.01, G_eff(4)=-0.04 so G(4)<G(3) (inverts, not widens); p_eff(parity) n-independent at 0.5 (no earlier flip with size); conjunctive collapses earlier than parity | probe_gap_scaling.py |
| 153 | Q8 | H4 | parity collapse tracks (1-2p)^n | PARTIAL: literal rule CONFIRMED (\|p_par*-p_pred\|=0.0006/0.0040 <=0.01) but degenerate (floor 2e-9 pins both to p~0.5); shape REFUTES - parity curve n-independent (0.880/0.763/0.542) vs n-dependent (1-2p)^n, residual 0.326/0.417 grows with n | probe_boolean_law.py |
| 154 | Q8-H5 | cliff vs glide (dΦ/dp shape) | REFUTED | parity & conjunctive both monotone glides, \|χ\| max at p→0, no interior peak at n=3 or n=4 (parity \|χ\|(0)=1.218/0.609; conj \|χ\|(0)=10.06/29.02); controls pass (instrument Φ=2.0 triadic; injected knee p=0.25 detected) |

## Reading across Q8 (probes 150–154)

Parity was supposed to behave differently from a conjunctive chain under flip noise, and on every test it does not behave the way the difference was predicted. The verdict-flip sweep (150) runs four curves — parity and conjunctive, each at n=3 and n=4 — and all four hold Φ_MIP above zero across the whole interior, going dyadic only at the p=0.5 endpoint. The parity collapse point sits at 0.50 for both sizes, exactly where conjunctive sits, so parity does not flip earlier in the interior. The normalized-decay probe (151) refutes the direction of the effect. Parity does not decay faster: it wins 0 of 49 interior points at both sizes, the conjunctive Φ̂ runs at or above parity everywhere, and the decay integral is larger for conjunctive (L_conj=5.16 at n=3, 10.17 at n=4) than for parity (L_par=2.47). Parity's normalized curve is the same at both sizes (0.763 at p=0.10 for n=3 and n=4) while conjunctive falls off faster, the reverse of the hypothesis. The gap-scaling probe (152) refutes the widening claim. The strict verdict gap is zero at both sizes because both forms flip only at p=0.5, and on the effective register the gap inverts rather than widens: G_eff(3)=−0.01 against G_eff(4)=−0.04, so the n=4 gap is smaller, and parity's effective collapse stays pinned at 0.5 regardless of size. The boolean-law probe (153) lands partial and explains why the literal fit is hollow. The predicted collapse point (1−2p)^n=0 matches the measured parity collapse to within 0.0006 and 0.0040, but only because a 2e-9 floor pins both the predicted and the measured collapse to p≈0.5, so the rule is confirmed on a degenerate point. The shape refutes it: the parity Φ curve is n-independent (0.880, 0.763, 0.542 at the sampled points) while (1−2p)^n is strongly n-dependent, and the residual grows with n (0.326 then 0.417). The cliff-versus-glide probe (154) closes the case. Both forms are smooth glides, not cliffs: |χ| takes its maximum as p→0 and shows no interior peak at either size (parity |χ|(0)=1.218 at n=3, 0.609 at n=4; conjunctive 10.06 then 29.02), while the controls confirm the test can see structure — the instrument reads Φ=2.0 triadic and an injected knee at p=0.25 is detected. Across the five: parity and conjunctive collapse at the same p=0.5 endpoint, parity's Φ is the slower-decaying and size-independent curve rather than the faster one, the predicted gap inverts with n instead of widening, and neither form has the interior cliff the parity rule was supposed to produce.

### Question Q9 — timescale_separation

| # | Concept (Q9) | Hypothesis | Verdict | Result |
|---|---|---|---|---|
| 155 | Q9-H1 | hold-for-k verdict flip, two_sided_match (W'=S,S'=W&C,C'=S), classify over k=1..6 | k*_det=2 interior; Φ_MIP 2.0→0.0 at k=2, dyadic & stays for k=3..6; instrument k=1 triadic Φ=2.0 MIP {W,SC} | CONFIRMED (timescale separation flips the verdict at a finite interior ratio, not magnitude-only) |
| 156 | Q9-H2 | hold-for-k membership on two_sided_match, k=1..6, major complex via new_big_phi.maximal_complex over reachable_states; phi_s,{S} vs phi_wc,{W,C} at most-integrated state | REFUTED | instrument control PASS (k=1 core {W,S,C}, Phi=2.0); at k*_det=2 the core contracts to {S} (state 000, phi_s=1.0 > phi_wc=0.0) and stays {S} through k=6 — the #43 direction: parties factor out, mediator becomes the self-absorbed core; core never reads {W,C} | org_frontier/questions/q9_timescale_separation/probe_hold_membership.py |
| 157 | Q9/H3 | construction split (hold-for-k vs prob-1/k) on two_sided_match, k=1..6 | k*_det=2 (Phi->0), k*_prob=none (prob Phi=2.0,0.271,0.138,0.092,0.069,0.055); ctrl k=1 both triadic Phi=2.0 {W,SC} | CONFIRMED — the slow-commit model is load-bearing | probe_construction_split.py |
| 158 | Q9-H4 | period-lock: k*_det vs synchronous attractor period (hold-for-k, S-only) | gig_false_dyad (P=1) k*_det=2; two_sided_match (P=2) k*_det=2; both Φ_MIP 2.0→0 at k=2 | REFUTED — both flip at the same period-independent k*_det=2; grain-2 hold spans a full cycle on the period-1 form, so the flip does not track P | probe_period_lock.py |
| 159 | Q9-H5 | clock-stretching vs #62 sequential factorization fingerprint (two_sided_match) | CONFIRMED (band only) | hold k*_det=2, dyadic band k=2..6 (contiguous), residual Φ_MIP=0.0; six #62 orders all dyadic Φ_MIP=0.0; grain-2 dyadic Φ_MIP=0.0; MIP-cut & residual-Φ identical ("None"/0.0) across all routes — only discriminator that fires is the temporal collapse band (hold holds over k=2..6 vs #62 per-order, no ratio band) | probe_factorization_fingerprint.py |

## Reading across Q9 (probes 155–159)

Slowing the mediator relative to the parties flips the verdict at a finite ratio, and it flips for a structural reason. The hold-for-k sweep (155) holds the commitment k clock steps and reads the verdict at each k: at k=1 the form is triadic with Φ_MIP=2.0 and MIP {W,SC}, and at k=2 Φ_MIP falls to 0.0 and the form goes dyadic and stays dyadic through k=6. The flip lands at an interior k*_det=2, so timescale separation, not noise magnitude, is what carries the verdict over. The membership probe (156) refutes the expected seat but exposes the mechanism. The major complex does not contract to the two parties {W,C} as k grows. It contracts to the mediator alone: at k*_det=2 the core is {S} at state 000 with phi_s=1.0 against phi_wc=0.0, and it stays {S} through k=6, the #43 direction in which the parties factor out and the mediator becomes the self-absorbed core. The construction probe (157) shows the slow-commit model is doing the work. Under the deterministic hold the verdict flips at k*_det=2, but under a probabilistic 1/k commit the form never flips — prob Φ runs 2.0, 0.271, 0.138, 0.092, 0.069, 0.055, decaying without reaching zero — so the collapse is a property of the hold construction and not of slowing per se. The period-lock probe (158) refutes the idea that the flip tracks the attractor period. The period-1 form (gig_false_dyad) and the period-2 form (two_sided_match) both flip at k*_det=2, and the grain-2 hold spans a full cycle on the period-1 form, so a period-independent k* governs the flip. The fingerprint probe (159) confirms a clock-stretching signature, but only as a band. Every route — the hold over k=2..6, the six #62 sequential orders, and the grain-2 collapse — produces the same dyadic Φ_MIP=0.0 with the same "None" MIP cut and the same zero residual, so the MIP-cut and residual-Φ discriminators cannot tell the temporal collapse from sequential factorization. The one feature that separates them is the contiguous k=2..6 collapse band, which the hold produces and the per-order #62 routes do not. Across the five: timescale separation flips the verdict at an interior k*=2 that does not depend on period, the slow-commit hold construction is what makes the flip happen, and the collapsed structure is the mediator-as-core, distinguishable from sequential factorization only by its ratio band, not by its cut or residual.

### Question Q10 — commit_delay

| # | Concept (Q10) | Hypothesis | Verdict | Result |
|---|---|---|---|---|
| 160 | Q10 H1 | buffer_pipeline / two_sided_match | d=0,1,2,3 | verdict triadic at all d (no flip), Φ_MIP=[2.0,1.0,2.0,2.0], dΦ/dd=[-1,+1,0]; pass-through verified (no self-edge); Φ rises at d1→2 → **REFUTED** (verdict-robust but not magnitude-monotone) | results/q10_h1_buffer_verdict.csv |
| 161 | Q10-H2 | buffer_pipeline two_sided_match d=1,2,3 micro-grain maximal_complex + buffer singleton phi + black-box re-read | REFUTED | d=1 core={S,C,B1} ejects W ({W,S,C} not recovered, Phi=1.0); d=2 core={W,S,C,B1,B2}, d=3 core={W,S,C,B1,B2,B3} — interior B_i NOT excluded though phi_{B_i}=0.0<1e-9; instrument d=0 triadic Phi=2.0 {W,S,C} PASS; black-box absorbs to dyadic {S} at d>=2 (grain branch) | probe_buffer_membership.py |
| 162 | Q10 H3 | construction split: buffer pipeline vs lagged read on two_sided_match, d=0..3 | CONFIRMED | disagree at d=1 (W-S-C complex buf{C,S} vs lag{W,S,C}) and d=2 (lag dyadic Φ=0 vs buf triadic Φ=2.0); lagged = single composed 3-node TPM, buffer = 3+d nodes; control Φ=2.0/{W,SC} passed | results/construction_split.csv |
| 163 | Q10/H4 | no d=2 threshold (buffer pipeline) | two_sided_match (P=2) & gig_false_dyad (P=1), d=0..3 | Φ_MIP(d)=[2,1,2,2] both forms; dΦ/dd=[-1,+1,0]; d=2 off d1-d3 line (resid 0.5>TOL); verdict triadic throughout | REFUTED — fd-discontinuity at d=2 for both forms (driven by a d=1 Φ dip-and-recovery, not a verdict flip) |
| 164 | Q10 H5 | probe_factorization_fingerprint.py | transport-delay (buffer pipeline) vs slowed-clock (Q9 hold-for-k @ k*_det=2) fingerprints on two_sided_match, matched grain | instrument ctrl PASS (triadic Φ=2.0 {W,SC}); transport triadic at d=1,2,3 (Φ=1.0,2.0,2.0), W-S-C cores {S,C}/{W,S,C}/{W,S,C}, never self-absorbed {S}, no buffer self-OR, MIP cuts {WS,CB1}/{WS,B1,CB2}/{WS,B1B2,CB3}; hold-for-k dyadic Φ=0 self-absorbed {S} core | CONFIRMED — distinct fingerprints (S-core watch, MIP-cut, pass-through all fire) |

## Reading across Q10 (probes 160–164)

A commit delay routed through pass-through buffers leaves the verdict standing and never reproduces the slowed-clock collapse. The buffer-verdict sweep (160) reads the form triadic at every delay d=0,1,2,3, with Φ_MIP running 2.0, 1.0, 2.0, 2.0 and the per-step change at −1, +1, 0. The hypothesis fails because Φ is not monotone in d: it dips at d=1 and recovers at d=2, while the pass-through is verified to carry no self-edge, so the delay is robust on the verdict but not on the magnitude. The membership probe (161) refutes the expectation that buffers stay out of the core. At d=1 the core is {S,C,B1} and ejects W, so the original {W,S,C} triad is not recovered at Φ=1.0; at d=2 and d=3 the core grows to include every interior buffer ({W,S,C,B1,B2} then {W,S,C,B1,B2,B3}) even though each B_i carries phi=0.0, so the zero-φ buffers are not excluded. The instrument reads d=0 triadic at Φ=2.0 on {W,S,C}, and the black-box re-read absorbs to the dyadic {S} only on the coarse-grain branch. The construction probe (162) shows the buffer model is load-bearing against a lagged read. The two constructions disagree at d=1, where the buffer complex is {C,S} and the lagged complex is {W,S,C}, and again at d=2, where the lagged form goes dyadic at Φ=0 while the buffer form stays triadic at Φ=2.0; the lagged read composes a single 3-node TPM while the buffer read adds 3+d nodes, and the control passes at Φ=2.0 on {W,SC}. The threshold probe (163) refutes a clean d=2 break by finding the same shape on both forms: Φ_MIP(d) runs 2,1,2,2 on the period-2 two_sided_match and the period-1 gig_false_dyad alike, with d=2 sitting off the d1–d3 line (residual 0.5 above tolerance) and the verdict triadic throughout, so the finite-difference discontinuity at d=2 is the d=1 dip-and-recovery, not a verdict flip. The fingerprint probe (164) confirms that transport delay and slowed clock leave different signatures. Transport delay holds the form triadic at d=1,2,3 (Φ=1.0,2.0,2.0) with W-S-C cores {S,C}, {W,S,C}, {W,S,C} that never self-absorb to {S} and never form a buffer self-OR, and MIP cuts that thread the buffers ({WS,CB1}, {WS,B1,CB2}, {WS,B1B2,CB3}), whereas the Q9 hold-for-k at k*_det=2 collapses to a dyadic Φ=0 self-absorbed {S} core. The S-core watch, the MIP-cut pattern, and the pass-through all fire to separate the two. Across the five: a buffered commit delay keeps the triad's verdict at every d while moving Φ non-monotonically, pulls the zero-φ buffers into the core rather than leaving them out, depends on the buffer construction to differ from a lagged read, breaks the d=2 line on both period forms through a dip-and-recovery rather than a flip, and carries a transport-delay fingerprint distinct from the slowed-clock collapse.

### Question Q11 — oscillatory_scaling

| # | Concept (Q11) | Hypothesis | Verdict | Result |
|---|---|---|---|---|
| 165 | Q11-H1 | rotating ring is a fifth Phi(n) law distinct from capped Phi=4 | CONFIRMED | rot_ring Phi_MIP=[2,2,2,2] over n=3..6 (constant, triadic, full core, period=n) vs and_ring=4.0 over n=4..6 (period<=2); gap -2.0 at every matched n; controls strict-med triad=2.0, and_ring(3)=6.0, and_ring(4)=4.0 all passed | org_frontier/questions/q11_oscillatory_scaling/probe_probe_q11_rotating_law.py |
| 166 | Q11-H2 | period_term | commit_ring(n,p) Delta_p Phi: n=3 -2.0, n=4 -3.0, n=5 -4.0, n=6 -5.0 (Phi=n-1 at p=1 -> 0 at p>=2); rot_ring corroborant flat Phi=2.0 at n=3..6; controls triad 2.0/and_ring(4)4.0/(3)6.0/parity_hub(5)0.125 PASS | CONFIRMED — period p is a term in the law: Phi moves when only p changes at every fixed n |
| 167 | Q11 H3 | flip_ring(n) NOT-of-left-neighbor vs parity_hub floor 2^(2-n), n=3..6 | CONFIRMED | Phi_flip=[2,2,2,2] (constant, full core) vs hub=[0.5,0.25,0.125,0.0625]; gaps widen [1.5,1.75,1.875,1.9375], ratios=1.0 (no halving); ctrl single_hub(3)=2.0 triadic, parity_hub(5)=0.125 full core | probe_probe_q11_flip_ring_decay.py |
| 168 | Q11 H4 | rot_ring & commit_ring(n,2) triadic + full core at n=3..6 | REFUTED | rot_ring triadic Phi=2.0 full core n=3,4,5; commit_ring(n,2) DYADIC Phi=0.0 core shed to {x0} at n=3,4,5 (collapses to copy-x0 map); IC and_ring(4)=4.0/and_ring(3)=6.0/strict-triad=2.0 all PASS | probe_probe_q11_triadic_persistence.py |
| 169 | Q11 H5 | grain band of rot_ring | g*(n) vs period p=n, grain g=1..n+1 | g*=[3,2,5,2] for n=3,4,5,6; not non-decreasing, g*(6)=2 < g*(3)=3; odd n flip at g=p, even n at g=2 (parity-of-n, not period-scaling) | REFUTED (instrument control PASS: ats_strict Φ=2.0, and_ring(4)=4.0, and_ring(3)=6.0, parity_hub(5)=0.125) |

## Reading across Q11 (probes 165–169)

A rotating ring carries its own Φ(n) law, and the law has a period term in it. The rotating-law probe (165) reads rot_ring at Φ_MIP=2.0 across n=3,4,5,6 — constant, triadic, full core, period equal to n — while the saturating and_ring sits at 4.0 over n=4,5,6, so the two forms hold a gap of −2.0 at every matched size and the rotating form is a distinct fifth law rather than a capped variant. The controls hold throughout (strict-mediator triad 2.0, and_ring(3)=6.0, and_ring(4)=4.0). The period-term probe (166) confirms that the period is a live variable in the law. Holding n fixed and moving only the commit period p changes Φ on commit_ring at every size: the Φ=n−1 value at p=1 drops to 0 once p≥2, giving step sizes of −2.0, −3.0, −4.0, −5.0 at n=3,4,5,6, while rot_ring stays flat at 2.0 as a corroborant. The flip-ring probe (167) confirms that a rotating NOT ring does not decay toward the parity-hub floor. Φ_flip holds 2.0 across n=3..6 with a full core while the parity hub halves down the 2^(2−n) floor (0.5, 0.25, 0.125, 0.0625), so the gap widens (1.5, 1.75, 1.875, 1.9375) and the ratio stays at 1.0 with no halving. The persistence probe (168) refutes the claim that the rotating structure survives a longer commit. rot_ring stays triadic at Φ=2.0 with a full core, but commit_ring(n,2) goes dyadic at Φ=0.0 and sheds the core down to {x0} at n=3,4,5, collapsing to a copy-x0 map. The grain-band probe (169) refutes a grain threshold that tracks the period. The flip grain g* runs 3, 2, 5, 2 across n=3,4,5,6, which is not non-decreasing — g*(6)=2 falls below g*(3)=3 — and the pattern keys to the parity of n (odd n flips at g=p, even n at g=2) rather than to period scaling, with the instrument controls passing (ats_strict Φ=2.0, and_ring(4)=4.0, and_ring(3)=6.0, parity_hub(5)=0.125). Across the five: the rotating ring is a separate Φ(n) law that holds 2.0 at full core where the saturating ring caps at 4.0, the commit period enters the law as a term that moves Φ at fixed n, a rotating NOT ring stays pinned at 2.0 instead of decaying to the parity-hub floor, the rotating structure does not survive a two-step commit, and the grain at which it collapses keys to the parity of n rather than to its period.
| 279 | Higher-order binding (Q120 H1) | No triadic form is a pure higher-order bind: every party is pivotal | **confirmed** | 0 of 2312 triadic forms survive any single-party knockout; all three parties pivotal in every triadic form across strict-mediation (24/256) and fully-coupled (2288/4096), under both spectator (P'=x[P]) and silenced (P'=0) knockouts; tightens the law to a biconditional on 3 parties. `q120_higher_order_binding/probe_higher_order_binding.py` |
| 280 | Higher-order binding at four parties (Q125 H1) | A pure higher-order bind (irreducible, no single pivotal party) exists at four parties | **refuted** | 0 of 267 irreducible four-party forms survive any single-party knockout; all four parties pivotal in every one across homogeneous-symmetric (14/16), curated-redundant (6/8), named-multiparty (3/7), and canonical-heterogeneous (244/256) families, under both knockout defs. Redundancy does not free the bind; extends Q120 to 4 parties. Majority-clique: parties outside the core {B,D} are still pivotal (pivotality != core membership). `q125_four_party_higher_order/probe_four_party_higher_order.py` |
| 281 | Interested mediator (Q126 H1-H2) | A system that imposes its own agenda over the parties strengthens (or erodes) the coordination | **H1,H2 confirmed (erodes)** | as the mediator overrides the parties joint determination toward its agenda, Φ falls to 0 and the parties leave the core; a denying agenda collapses the triad at the first override (overriding the parties point of agreement), an approving one tolerates one (Φ 2.0->0.5->0); order-averaged, deny decays linearly (2,1.5,1,0.5,0) while approve is non-monotone (parity re-integration). Self-interest is disintegrating, not strengthening; Φ falls not rises. `q126_interested_mediator/probe_interested_mediator.py` |
| 282 | Interest collapse vs baseline (Q127) | The agenda that collapses coordination fastest tracks the mediators minority output-class, not denial | **H1 refuted, refined finding confirmed** | denial is not special: AND->deny collapses at k*=1 but OR->approve at k*=1 (asymmetry flips with the baselines rare output); balanced baselines (XNOR/XOR) re-integrate -- a faithful Φ=0.5 mediator is sharpened by self-interest, Φ rising to 2.0 before collapsing at full imposition. Corrosiveness depends on how discriminating the faithful mediation already was. `q127_interest_baselines/probe_interest_baselines.py` |
| 283 | Adaptive interested mediator (Q128) | Does a predatory mediator (S=O) re-integrate the coordination when its objective adapts to the parties? | **refined: iff it reads both** | a fully self-executing mediator is dyadic when its objective is frozen or reads one party, triadic iff the objective encodes both (W&C/W|C/W^C); the objective is always the core conduit (W,C->O->S->W,C), and under XOR all four nodes enter the core. Self-interest and irreducibility coexist only through an objective learned from both sides. `q128_adaptive_mediator/probe_adaptive_mediator.py` |
| 284 | Mediator interpolation faithful->predatory (Q129) | Does an adaptive objective hold the coordination across the interpolation? | **refined: two readings diverge** | parties-bound-in-core reading: frozen holds to m=3, adaptive breaks at m=1 (displaces the worker); whole-system reading (Q128): frozen never irreducible, adaptive only at m=4 (core SCO). An adaptive objective re-integrates the SYSTEM by joining the core and displacing a party; a frozen one preserves the PARTIES bind until full defection. "Coordination survives" has no single answer. `q129_mediator_interpolation/probe_mediator_interpolation.py` |
| 285 | Pivotal but excluded (Q130) | Are pivotality (removal collapses irreducibility) and core membership (in the major complex) the same? | **H1,H2 confirmed: distinct** | in the homogeneous symmetric 4-party forms, 14 parties are pivotal yet outside the major complex (necessity != membership); the core reads all 14; 0 core-but-not-pivotal, so core membership is a strict subset of pivotality. Mechanism: symmetry degeneracy (the major complex names one of several equivalent maximal complexes). The static form of Q129 displacement / influence-not-membership. `q130_pivotal_excluded/probe_pivotal_excluded.py` |
| 286 | Value capture under an interested mediator (Q131) | Does an interested mediator extract a larger share (rent) or destroy value (incl. its own)? | **H1,H2 confirmed: destruction** | Shapley value of subsystem-Φ across the Q126 interestedness ladder: faithful mediator captures 2/3 (1.333 of Φ=2.0); at the first interested step the value halves AND the split equalizes (W/S/C all 0.167, mediator share 67%->33%), then collapses to 0 at k=2. Interested mediation is value destruction not rent extraction -- the mediator surrenders the bottleneck position that gave it 2/3. Refines Q111 (the 2/3 is contingent on faithfulness). `q131_value_capture/probe_value_capture.py` |
| 287 | Value capture across baselines (Q132) | Is destruction-vs-extraction baseline-relative, and does the value reading need the integrating state? | **H2 confirmed (baseline-relative); H1 refined** | verdict-aligned Shapley: AND/OR destroy (2/3 rent falls with Φ); XOR re-integrates under self-interest (Φ 0.5->2.0) and the mediator captures 2/3 -- EXTRACTION. The 2/3 rent tracks full integration, so self-interest extracts where it re-integrates (XOR) and destroys where it collapses (AND/OR) -- the value image of Q127. Methodological: the all-ones background (Q111) misses OR value and XOR re-integration, isolating the Q122 background-state problem. `q132_value_baselines/probe_value_baselines.py` |
| 288 | Deny-agenda value symmetry (Q133) | Does the destruction-vs-extraction value split mirror under the deny agenda? | **H1,H2 confirmed: agenda-symmetric** | under deny the extracting baseline is XNOR (Φ 0.5->2.0, mediator 2/3) -- the mirror of XOR-under-approve (Q132); AND/OR destroy, XOR stays weak. Each agenda re-integrates the balanced baseline whose minority output it overrides; re-integration is extraction. Value image of Q127 agenda symmetry. `q133_deny_symmetry/probe_deny_symmetry.py` |
| 289 | Does the mediator rent scale? (Q134) | Is the mediator two-thirds share scale-invariant as parties are added? | **H1 refuted: rent dilutes** | conjunctive star n=3,4,5: total Φ=n-1 but mediator share falls 66.6%->58.3%->55.0% toward 1/2; outer parties collectively keep more (33%->45%) though each keeps less. The 2/3 is a 3-party number, not a law of bottlenecks; concentration is a small-coordination phenomenon. Bounds the Q111 platform-power reading. `q134_rent_scaling/probe_rent_scaling.py` |
| 290 | The objective rent (Q135) | When the systems agenda joins the core, does it capture value and at whose expense? | **H1 confirmed; co-equal rentier** | Shapley of the Q128 adaptive-objective model: the objective captures exactly what the system does (O-share = S-share) in every adaptive form -- AND/OR 58% each, XOR 25% each. The parties value tracks Q129 displacement: negative under AND/OR (objective displaces a party, core SCO), positive equal-quarter under XOR (all four bound). The platform agenda is a rentier on a par with the platform. `q135_objective_rent/probe_objective_rent.py` |
| 291 | Two competing mediators (Q136) | Does a second mediator (competition) return value to the parties? | **H1,H2 confirmed: no** | substitutable mediators (parties read S1|S2) factor the coordination -- whole-system dyadic Φ=0, major complex a degenerate {S2,C} fragment, no rent for anyone; complementary mediators (read S1&S2) raise Φ to 4.0 split evenly (each of 4 takes 25%, each mediator 25%<2/3, each party 25%>1/6). Competition either destroys the coordination or stacks another toll -- it does not return the platform cut to the parties. `q136_competing_mediators/probe_competing_mediators.py` |
| 292 | The interested worker (Q137) | Can the worker reclaim value by gaming the system (algorithmic resistance)? | **H1,H2 confirmed: no** | sweeping system x worker interestedness: worker gaming collapses the coordination (Φ->0 for any k_W>=1) -- the worker reads only the system, so injecting an agenda makes it unresponsive and the triad factors. The workers share rises 17%->33% only when the SYSTEM defects (Q131 equalization) but its absolute value falls 0.333->0.167 and never exceeds baseline. Resistance breaks the bind; it does not redistribute. Worker/system asymmetry: the system reads two parties and survives selective interest, the worker reads one and can only break it. `q137_interested_worker/probe_interested_worker.py` |
| 293 | Minimum faithful fraction (Q138) | How much forced faithfulness revives the coordination, and who gets the value? | **H1,H2 confirmed** | predatory mediator forced faithful on m/4 states: below 3/4 the coordination is dead (Φ=0); at 3/4 it revives with an EGALITARIAN split (Φ=0.5, mediator 33%, parties 67%); the mediator 2/3 returns only at full faithfulness (4/4, Φ=2.0). The platform rent is the fully-faithful commit -- minimum-viable regulation revives the coordination without restoring platform dominance; the last increment of faithfulness concentrates the rent. `q138_faithful_fraction/probe_faithful_fraction.py` |
| 294 | Value and veto under interest (Q139) | Is Q112 decoupling (equal veto, unequal value) fixed, or does it move with interest? | **H1,H2 confirmed** | veto is universal (3/3) at every interestedness level with positive Φ -- Q112 democratic destruction is robust to interest. The value-veto gap (mediator share - 1/3) tracks integration not veto: +1/3 at Φ=2.0, 0 at Φ=0.5, independent of baseline/agenda. Destructive interest (AND) democratizes value to match the equal veto (gap closes); re-integrating interest (XOR) re-concentrates it (gap opens). Q112 decoupling IS the integration. `q139_value_veto/probe_value_veto.py` |
| 295 | Coalition stability under interest (Q140) | What does the cooperative-game core of the mediated triad look like, and what does interest do to it? | **H1,H2 confirmed** | faithful core is a single point: mediator takes 100% (not the Shapley 2/3), parties 0, because either party with the mediator reaches full value (v(WS)=v(SC)=v(N)=2) so each party is substitutable and has no stable claim. Interested mediator destroys the sub-coalition values (all 0) -> core opens to the whole simplex, parties can keep the entire reduced value (0.5). Interest shrinks the pie but frees its division; faithful mediation grows it but monopolizes it. `q140_coalition_stability/probe_coalition_stability.py` |
| 296 | The lagging objective (Q141) | Does adaptation on a delay rescue the party that immediate adaptation displaces (Q129)? | **H1,H2 confirmed** | immediate objective (O=W&C, 4 nodes) displaces the worker -- core {S,C,O} Φ=1.0; a lagging objective (O=M, M=W&C, 5 nodes) re-integrates the worker -- core {W,S,C,O,M} Φ=2.0, all five bound. The timescale of the systems learning decides the workers place: real-time adaptation substitutes for the worker and pushes it out; slow learning (a memory of the joint state) keeps it constitutive. Slower learning is structurally more inclusive; displacement is immediacy, not self-interest alone. `q141_lagging_objective/probe_lagging_objective.py` |
| 297 | q144 mediator hierarchy depth | workflow study | confirmed | H1 depth-is-flat-at-chain-constant: SUPPORTED | q144_mediator_hierarchy_depth/probe_mediator_hierarchy_depth.py |
| 298 | q145 spanning mediator law | workflow study | confirmed | H1 spanning-mediator law (full span holds all, Φ linear in n; partial caps at span+1): SUPPORTED | q145_spanning_mediator_law/probe_spanning_mediator_law.py |
| 299 | q148 multihub chain hierarchy | workflow study | confirmed | H1 (depth caps integrable group size): SUPPORTED | q148_multihub_chain_hierarchy/probe_multihub_chain_hierarchy.py |
| 300 | q150 ring chord integration | workflow study | refuted/refined | H1 chord raises ring Φ and shifts MIP cut off the chord: REFUTED | q150_ring_chord_integration/probe_ring_chord_integration.py |
| 301 | q151 hub failure resilience | workflow study | confirmed | H1 (redundant mediation buys verdict resilience): SUPPORTED | q151_hub_failure_resilience/probe_hub_failure_resilience.py |
| 302 | q183 build disagreement bridge | workflow study | confirmed | H1 zero-anchored spread on identical accounts: SUPPORTED | q183_build_disagreement_bridge/probe_build_disagreement_bridge.py |
| 303 | q143 ring verdict geometry | workflow study | refuted/refined | H1 ring is triadic with a single balanced two-arc cut of constant Φ in n: REFUTED | q143_ring_verdict_geometry/probe_ring_verdict_geometry.py |
| 304 | q146 smallworld rewire phi | workflow study | refuted/refined | H1 non-monotone small-world Φ peak: REFUTED | q146_smallworld_rewire_phi/probe_smallworld_rewire_phi.py |
| 305 | q147 random boolean topology | workflow study | confirmed | H1 (recurrence topology beats mean degree at predicting triadicity): SUPPORTED | q147_random_boolean_topology/probe_random_boolean_topology.py |
| 306 | q149 topology shapley split | workflow study | refuted/refined | H1 (per-hub Shapley share falls monotonically as mediation distributes): REFUTED | q149_topology_shapley_split/probe_topology_shapley_split.py |
| 307 | q152 verdict vs complex topology | workflow study | confirmed | H1 verdict-complex disagreement exists: SUPPORTED | q152_verdict_vs_complex_topology/probe_verdict_vs_complex_topology.py |
| 308 | q153 triadic dyadic classifier | workflow study | refuted/refined | H1 CRQA features beat the majority baseline: REFUTED | q153_triadic_dyadic_classifier/probe_triadic_dyadic_classifier.py |
| 309 | q154 bottleneck node recovery | workflow study | refuted/refined | H1 (coupling centrality recovers the structural bottleneck in >70% of seeded runs): REFUTED | q154_bottleneck_node_recovery/probe_bottleneck_node_recovery.py |
| 310 | q155 mc membership threshold | workflow study | refuted/refined | H1 (single coupling-centrality threshold recovers membership at pooled AUC>0.65): REFUTED  [AUC=0.649, null=0.494] | q155_mc_membership_threshold/probe_mc_membership_threshold.py |
| 311 | q156 interested mediator signature | workflow study | refuted/refined | H1 (interested mediators show lower outgoing prominence, separating >70% of pairs): REFUTED | q156_interested_mediator_signature/probe_interested_mediator_signature.py |
| 312 | q157 te vs dcrp direction | workflow study | confirmed | H1 (TE beats DCRP peak-lag by > 5 pp on directed-edge orientation): SUPPORTED [TE 77.2% - DCRP 56.5% = +20.7 pp] | q157_te_vs_dcrp_direction/probe_te_vs_dcrp_direction.py |
| 313 | q158 phi magnitude regression | workflow study | refuted/refined | H1 DET correlates positively with major-complex Φ (rho>0.4): REFUTED | q158_phi_magnitude_regression/probe_phi_magnitude_regression.py |
| 314 | q159 length to verdict curve | workflow study | confirmed | H1 CRQA verdict converges by 600 steps for >80% of forms with no gain past 1200: SUPPORTED | q159_length_to_verdict_curve/probe_length_to_verdict_curve.py |
| 315 | q160 five node complex recovery | workflow study | confirmed | H1 (five-node full-separation fraction is LOWER than the four-node 36% rate, dissociation widens with scale): SUPPORTED | q160_five_node_complex_recovery/probe_five_node_complex_recovery.py |
| 316 | q161 noise robust verdict | workflow study | refuted/refined | H1 (triad/dyad robust > 80% of forms AND bottleneck degrades > 20 pts): REFUTED | q161_noise_robust_verdict/probe_noise_robust_verdict.py |
| 317 | q162 feature ablation minimal | workflow study | refuted/refined | H1 (coupling prominence A within 3 pts of full CRQA A+B): REFUTED | q162_feature_ablation_minimal/probe_feature_ablation_minimal.py |
| 318 | q163 computationalism under agenda | workflow study | confirmed | H1 surplus rises then collapses (interested actor is a third object): SUPPORTED  [approve rises, deny monotone] | q163_computationalism_under_agenda/probe_computationalism_under_agenda.py |
| 319 | q164 perceivable agenda fraction | workflow study | refuted/refined | H1 (agenda LESS perceivable than a hidden rule: AUC 0.96 < 0.67): REFUTED | q164_perceivable_agenda_fraction/probe_perceivable_agenda_fraction.py |
| 320 | q165 intent compression into agenda | workflow study | confirmed | H1 (interested Φ(q) strictly below faithful at every q<1 — interest steepens embodiment loss): SUPPORTED | q165_intent_compression_into_agenda/probe_intent_compression_into_agenda.py |
| 321 | q166 phantom addressee displaced | workflow study | confirmed | H1 (the addressee inverts: W exits the core for an agenda-governed bind as k rises): SUPPORTED | q166_phantom_addressee_displaced/probe_phantom_addressee_displaced.py |
| 322 | q167 capture meets agenda | workflow study | refuted/refined | H1 (an interested platform displaces the worker at a lower g* than a faithful one, monotonically in interest): REFUTED | q167_capture_meets_agenda/probe_capture_meets_agenda.py |
| 323 | q168 opacity floor under agenda | workflow study | confirmed | H1 interest raises H(out\|W) above the 0.50-bit faithful floor: SUPPORTED | q168_opacity_floor_under_agenda/probe_opacity_floor_under_agenda.py |
| 324 | q169 lagging objective prediction lag | workflow study | confirmed | H1 (lagging objective predicts the platform better at matched drift): SUPPORTED | q169_lagging_objective_prediction_lag/probe_lagging_objective_prediction_lag.py |
| 325 | q170 extracting platform tracking | workflow study | confirmed | H1 (recovered-model KL is higher for the interested mediator than the faithful one): SUPPORTED | q170_extracting_platform_tracking/probe_extracting_platform_tracking.py |
| 326 | q171 drift binding with agenda | workflow study | refuted/refined | H1 (drift and agenda super-additively destroy the binding): REFUTED | q171_drift_binding_with_agenda/probe_drift_binding_with_agenda.py |
| 327 | q172 facet difficulty under interest | workflow study | refuted/refined | H1 (interest is facet-selective, concentrated on the named survey facets): REFUTED | q172_facet_difficulty_under_interest/probe_facet_difficulty_under_interest.py |
| 328 | q173 build rule to phi | workflow study | confirmed | H1 verdict reproducibility + degenerate CI under agreement: SUPPORTED | q173_build_rule_to_phi/probe_build_rule_to_phi.py |
| 329 | q184 gig dispatch spread | workflow study | confirmed | H1 driver-dyad vs platform-triad split, phi_gap = platform Φ (2.000000): SUPPORTED | q184_gig_dispatch_spread/probe_gig_dispatch_spread.py |
| 330 | q185 ats commit convey spread | workflow study | confirmed | H1 commit/convey spread on identical topology: SUPPORTED | q185_ats_commit_convey_spread/probe_ats_commit_convey_spread.py |
| 331 | q186 spread decomposition law | workflow study | confirmed | H1 three components not rank-one collinear (both off-diagonal cells non-empty: 14, 1; some rho<1: True): SUPPORTED | q186_spread_decomposition_law/probe_spread_decomposition_law.py |
| 332 | q187 moderation oversight spread | workflow study | confirmed | H1 disagreement localizes to the policy-team node: SUPPORTED | q187_moderation_oversight_spread/probe_moderation_oversight_spread.py |
| 333 | q188 handoff directionality spread | workflow study | confirmed | H1 one-way dyadic / reciprocal triadic, I in core only under reciprocal: SUPPORTED | q188_handoff_directionality_spread/probe_handoff_directionality_spread.py |
| 334 | q190 spread robustness noise | workflow study | confirmed | H1 (flips occur only near the dyad/triad boundary, never far from it): SUPPORTED | q190_spread_robustness_noise/probe_spread_robustness_noise.py |
| 335 | q193 phi bridge build | workflow study | confirmed | H1 (Φ_coord predicts ACS-total, r>0.20 & CI excludes 0): SUPPORTED | q193_phi_bridge_build/probe_phi_bridge_build.py |
| 336 | q174 alpha phi width | workflow study | confirmed | H1 monotone CI width tracks agreement: SUPPORTED | q174_alpha_phi_width/probe_alpha_phi_width.py |
| 337 | q175 substitutability flipper | workflow study | confirmed | H1 substitutable pool drives max_phi to 0 (dyadic) for all k>=2: SUPPORTED | q175_substitutability_flipper/probe_substitutability_flipper.py |
| 338 | q176 passthrough flipper | workflow study | refuted/refined | H1 pass-through flip is universal (100% flip rate): REFUTED | q176_passthrough_flipper/probe_passthrough_flipper.py |
| 339 | q177 spectator complex | workflow study | confirmed | H1 (idle spectator leaves the triadic core intact in >95% of accounts): SUPPORTED (core stable in 1.000 of triadic-core accounts) | q177_spectator_complex/probe_spectator_complex.py |
| 340 | q178 bit grain sensitivity | workflow study | confirmed | H1 threshold flips verdict for >20% of accounts (flip rate 0.3300): SUPPORTED | q178_bit_grain_sensitivity/probe_bit_grain_sensitivity.py |
| 341 | q179 time grain sensitivity | workflow study | confirmed | H1 (>15% of triadic accounts flip under the 2-tick grain): SUPPORTED | q179_time_grain_sensitivity/probe_time_grain_sensitivity.py |
| 342 | q180 individuation boundary | workflow study | confirmed | H1 splitting flips the verdict for >20% of accounts (flip rate 0.452): SUPPORTED | q180_individuation_boundary/probe_individuation_boundary.py |
| 343 | q181 flipper interaction | workflow study | confirmed | H1 flippers do not interact to restore irreducibility (no triadic re-emergence): SUPPORTED | q181_flipper_interaction/probe_flipper_interaction.py |
| 344 | q182 adversarial coding | workflow study | confirmed | H1 adversary flips point verdict on >40% of attacked accounts: SUPPORTED | q182_adversarial_coding/probe_adversarial_coding.py |
| 345 | q189 peer review veto spread | workflow study | confirmed | H1 gate/conduit spread puts editor core membership in dispute (core_jaccard<1): SUPPORTED | q189_peer_review_veto_spread/probe_peer_review_veto_spread.py |
| 346 | q191 triage spectator spread | workflow study | confirmed | H1 unread spectator leaves no Φ trace (spread=0 despite membership disagreement): SUPPORTED | q191_triage_spectator_spread/probe_triage_spectator_spread.py |
| 347 | q192 spread reconciliation cost | workflow study | refuted/refined | H1 edit distance increases with the Φ spread: REFUTED | q192_spread_reconciliation_cost/probe_spread_reconciliation_cost.py |
| 348 | q194 phi facet specificity | workflow study | confirmed | H1 (Φ predicts CI more than SE; Δr CI excludes 0): SUPPORTED | q194_phi_facet_specificity/probe_phi_facet_specificity.py |
| 349 | q195 phi incremental over se | workflow study | confirmed | H1 (partial r(Φ,ACS\|SE,BE) > 0.15 & CI excludes 0): SUPPORTED | q195_phi_incremental_over_se/probe_phi_incremental_over_se.py |
| 350 | q196 phi commit moderation | workflow study | confirmed | H1 (Φ_coord×SA-commit interaction on ACS-total is positive, CI excludes 0): SUPPORTED | q196_phi_commit_moderation/probe_phi_commit_moderation.py |
| 351 | q197 phi invariance waves | workflow study | confirmed | H1 (metric invariance, slope equal across W1-W3, ΔCFI<= 0.01): SUPPORTED | q197_phi_invariance_waves/probe_phi_invariance_waves.py |
| 352 | q198 phi predicts growth | workflow study | confirmed | H1 (W1 Φ_coord predicts the ACS-growth slope, β>0 & CI excludes 0): SUPPORTED | q198_phi_predicts_growth/probe_phi_predicts_growth.py |
| 353 | q199 phi growth mechanism | workflow study | confirmed | H1 (within-person ΔΦ tracks ΔACS, within slope > 0 & CI excludes 0): SUPPORTED | q199_phi_growth_mechanism/probe_phi_growth_mechanism.py |
| 354 | q200 phi bifactor loadings | workflow study | confirmed | H1 (Φ_coord loads on the GENERAL factor, β>0 & CI excludes 0, and the general path exceeds each specific-facet path): SUPPORTED | q200_phi_bifactor_loadings/probe_phi_bifactor_loadings.py |
| 355 | q201 phi substitutability capture | workflow study | confirmed | H1 (SU lowers Φ_coord, CI<0; lower Φ_coord predicts lower ACS, CI>0): SUPPORTED | q201_phi_substitutability_capture/probe_phi_substitutability_capture.py |
| 356 | q202 phi bridge falsification | workflow study | confirmed | H1 (worker-form shuffle destroys every Φ_coord-to-ACS relation; real r outside null at p<.05 in all cohorts): SUPPORTED | q202_phi_bridge_falsification/probe_phi_bridge_falsification.py |
| 357 | q203 real coordination coupling | real-data study | mixed | CRQA reads structure (%DET 59.8); TE/Granger/CCM agree on direction (P2->P1) but none significant — behavioral coupling gives no confident verdict on a real dyad | q203_real_coordination_coupling/probe_real_coordination_coupling.py |
| 358 | q204 phi on real coordination | real-data study | mixed | first exact Phi on a real coordination (eyemovement gaze); per-person codings factorize (Phi~0) while a folded coding manufactures integration (Phi 0.53) — the verdict is coder-dependent on real data | q204_phi_on_real_coordination/probe_phi_on_real_coordination.py |
| 359 | q205 latency feedback | computational | confirmed | represented latency keeps the triad triadic (whole-system Phi 2.0->1.0, buffer joins the core {S,C,B} and the worker drops out); the same dynamics read one-step over (W,S,C) only factor (dyadic, Phi~0) while the same estimation on the undelayed triad stays triadic (Phi 0.765) — one-step Phi misses lag only when the delay is unmodeled, formalizing the q204 caveat | q205_latency_feedback/probe_latency_feedback.py |
| 360 | q206 ring parity rewire | computational | mixed | coupling family governs the rewiring response: the conjunctive ring's decline is non-monotone on a finer grid (dyadic window at p=0.35-0.40, triadic recovery at 0.45-0.50; H2 refuted, q146's monotone result is a coarse-grid artifact), while a parity (XOR) ring starts 8x less integrated (Phi 0.5 vs 4.0, H3) yet never factors under rewiring (triadic at p=0,0.35,1.0; H4 refuted, H5 confirmed) — AND can be driven constant and factor, XOR stays sensitive to its inputs | q206_ring_parity_rewire/probe_ring_parity_rewire.py |
| 361 | q207 slow mediator | computational | mixed | a half-rate mediator (clock-gated, recomputes every 2nd step) does NOT bind: the triad factors (dyadic, Phi 0) and the only complex left is the clock's own 2-cycle {K} Phi 1.0 (H2,H3 refuted; H4,H5 confirmed; frozen mediator gives the identical result) — contrast q205: delaying when a commitment ARRIVES keeps the triad triadic, but reducing how OFTEN the mediator commits dissolves it. Commit rate is load-bearing, arrival time is not | q207_slow_mediator/probe_slow_mediator.py |
| 362 | q208 latency depth | computational | mixed | sweeping represented-latency depth k=0..3 (buffers): the triad stays triadic at every depth (H2) but whole-system Phi_MIP is NON-monotone (2.0,1.0,2.0,2.0) — the k=1 worker-drop and Phi-halving q205 reported is a depth-1 ANOMALY (H3,H5 refuted), not a decay law; at k>=2 the worker rejoins and the whole delay line + triad is one complex at Phi=2.0 (H4). Represented delay is benign for irreducibility; lateness doesn't break binding (cf q207, where commit rate does) | q208_latency_depth/probe_latency_depth.py |
| 363 | q209 commit period | computational | confirmed | period-sweep of the commit cadence (p=1..4 via mod-p counter): synchronous commitment is the UNIQUE binding cadence — p=1 triadic Phi=2.0, every p>=2 dyadic Phi=0 with no threshold/recovery (H2,H3,H5). The surviving complex is period-INVARIANT: always a single self-toggling bit at Phi=1.0 (the 2-bit counter never forms an integrated pair, carry is one-way), not growing with period (H4 flat). Completes the q208 contrast: latency depth flat+benign, commit period flat+fatal — lateness is free, intermittency at any degree is total | q209_commit_period/probe_commit_period.py |
| 364 | q210 shared counterpart | computational | mixed | two conjunctive triads sharing one counterpart do NOT merge into one core (all 4 hypotheses refuted): no bridge rule (none/AND/OR) makes the major complex span both triads or exceed the single-triad Phi=2.0 — the maximal complex stays one local triad {W1,S1,C} or a tie-broken pair {W2,S2}. The bridge rule controls the WHOLE-SYSTEM verdict instead (AND triadic Phi_MIP=2.0, none/OR dyadic 0), not core merger. A single shared peripheral party is too weak a link to bind two triads | q210_shared_counterpart/probe_shared_counterpart.py |
| 365 | q211 direct mediator channel | computational | confirmed | the complement of q210: two complete triads joined only by a DIRECT mediator-mediator channel (S1 reads S2, S2 reads S1; none/AND/OR) DO merge (all 5 hypotheses confirmed). AND channel -> whole triadic Phi_MIP=2.0, major complex {S1,W2,S2,C2} Phi=3.0 spanning both triads and super-additive (>2.0); OR -> whole factors but mediators bind into {S1,S2} Phi=2.0 across the boundary. The q210 obstacle was the INDIRECT path, not the principle: a direct link between the parts carrying each triad's integration (the mediators) merges what a shared counterpart could not | q211_direct_mediator_channel/probe_direct_mediator_channel.py |
| 366 | q212 channel location | computational | confirmed | localizes the q211 merger by moving the same AND cross-triad channel across three homologous node pairs: ONLY the mediator-mediator channel merges (core {S1,W2,S2,C2} spans both triads, Phi=3.0). Worker-worker and counterpart-counterpart channels do NOT merge — core stays inside one triad at Phi=2.0, and the two leaf positions agree (worker/counterpart symmetric). All three placements make the WHOLE system triadic (Phi_MIP=2.0), but core merger happens only at the integrating node: whole-system irreducibility is cheap, core merger is not, and channel location decides the latter | q212_channel_location/probe_channel_location.py |
| 367 | q213 contingent irreducibility | computational | confirmed | NEW CATEGORY + reusable classifier (classifier/contingency.py): a party can be in the core because it integrates (intrinsic) or because a constraint forbids the bypass around it (contingent — the car dealer: a pure relay, irreducible only because franchise law forbids the maker selling direct). The bypass-counterfactual restores the forbidden edge and reclassifies: car dealer -> contingent (leaves core, dyadic, margin=full Phi 2.0); conjunctive clearinghouse under the SAME op -> intrinsic (stays, margin 0); clearinghouse+back-channel -> partial (stays, margin 1.585); free conduit -> reducible (never in core). All 5 H supported. Distinct from q96 (state-contingent, resolves to optionality); the exception to the conduit-factors law | q213_contingent_irreducibility/probe_contingent_irreducibility.py |
| 368 | q214 triadic classification | computational | confirmed | applies the q213 bypass-counterfactual to 19 canonical triad types from the brokerage/sociology/economics literature (Simmel, Gould-Fernandez, Obstfeld, Burt, Quintane-Carnabuci, Rochet-Tirole). The verbal gaudens/iungens line IS the formal contingent/necessary line: all 5 gaudens-family brokers (gaudens, separans, divide-et-impera, structural-hole, bridge) -> contingent margin 2.0; integrators (Simmelian mediator, two-sided platform) -> necessary margin 0. REFINEMENT: tertius iungens splits — integrating -> necessary, self-liquidating (fully joins its parties = creates the bypass) -> reducible, a split the verbal theory conflates. Gould-Fernandez 5 roles sort by group boundary. BOUNDARY: Heider balance triads are mutual not mediated (Phi=6, no forbidden edge) — test N/A. All 5 H supported. Stage-2 deep research, 19 cited sources | q214_triadic_classification/probe_triadic_classification.py |
| 369 | thinkers/simmel H1 superindividual triad | computational | confirmed | Simmel (1902, I: 45): the dyad has no "superindividual energy"; the triad has, via the "broken line" through the third. Mutual dyad Phi=2.0 {A,B}; mutual triad Phi=6.0 {A,B,C}; direct A-B tie removed (A'=C, B'=C, C'=A&B) still binds A and B through C at Phi=2.0 — Simmel's broken line IS the lab's conjunctive-mediator control; cut dyad Phi=0, a single self-reading element remains. `thinkers/simmel/probe_simmel_superindividual.py` |
| 370 | thinkers/simmel H2 the decisive step | computational | refuted | Simmel: the 2->3 step changes the group in kind, later steps in degree. Mutual conjunctive cliques n=2..5: Phi = 2, 6, 12, 20 = n(n-1) (the count of directed ties); increments +4, +6, +8 GROW, so the 2->3 step is not distinguished on the pre-registered absolute rule. Post hoc: the proportional jump (x3, x2, x1.67) is largest at 2->3. `thinkers/simmel/probe_simmel_number.py` |
| 371 | thinkers/simmel H3 the majority | computational | refuted | Simmel: at three a majority "can override the individual" — the triad's mechanism of superindividuality. A triad where each member follows maj(A,B,C) has NO irreducible structure: Phi_MIP=0 and no complex at any state, not even a pair; unanimity (X'=A&B&C) binds at Phi=6.0 {A,B,C}. Extends probe 10 (mediated majority factors) to the mutual form, more strongly: majority = every member dispensable = substitutability collapse (finding 5). `thinkers/simmel/probe_simmel_majority.py` |
| 372 | thinkers/simmel H4 mediator or arbitrator | computational | partial | Simmel's nonpartisan scale: arbitrator (parties adopt M's decision) in core Phi=2.0; mediator (parties keep their will, A'=A&M, read only M) ALSO in core at Phi=2.0 — no "intermediate grade"; self-eliminated mediator (parties unite directly, M unread) out, whole factors, pair remains at 2.0 (cf. q214's self-liquidating iungens). Refines probe 21: contestability that bypasses the third factors, contestability that runs through him does not. `thinkers/simmel/probe_simmel_nonpartisan.py` |
| 373 | thinkers/simmel H5 tertius gaudens | computational | confirmed | Simmel: the third's leverage is set "exclusively" by the balance between the two. Weighted outcome O'=[wA*A + wT*T > wB*B + wT*(1-T)], A'=B'=T'=O, only weights vary: balanced (1,1,1) infl(T)=.75, core {T,O} Phi=2.0; intermediate (2,1,1) infl .5, core {T,O} 0.277; dictator (3,1,1) infl .25, T out, core {A,O} 2.0. Membership tracks pivotality (probes 11, 16). ADDITION: under balance the contestants are OUTSIDE the core — the irreducible whole is whoever holds the decision plus the decision. `thinkers/simmel/probe_simmel_gaudens.py` |
| 374 | thinkers/peirce H1 triadic irreducibility | computational | refuted | Peirce (CP 2.274): a genuine triad's members are bound "in a way that does not consist in any complexus of dyadic relations." Rendering: a relation = an irreducible IIT-4.0 distinction; its adicity = \|mechanism ∪ purview\| (max of cause/effect side). All 89 one-input copy wirings (8 at n=3, 81 at n=4): adicity {2: 11, 3: 66, 4: 12} — REFUTED as registered. POST HOC: the 11 at adicity 2 are exactly the unbranched wirings (pure cycles + products of mutual dyads); every wiring where a node is read by >=2 others reaches 3 (a hub read by 3 reaches 4: D→{A,B,C} φ=1.0), ALL on the effect side; max CAUSE-side adicity over all 89 = 2. Herzberger's valency argument holds for "determined by"; Burch's teridentity (branching = triad) holds for fan-out. Φ>0 in exactly the 8 pure cycles (Φ=2.0, full core, cf. probe 168), all at adicity 2; all 78 branched wirings Φ=0. `thinkers/peirce/probe_peirce_irreducibility.py` |
| 375 | thinkers/peirce H2 grades of degeneracy | computational | confirmed | Peirce (CP 1.473): every triad is monadically degenerate, dyadically degenerate, or genuine. His exemplars order exactly by adicity: three monads (A'=A,B'=B,C'=C) 1; "A father of B, B father of C" (B'=A, C'=B) 2; the conjunctive control 3. Degenerate forms Φ=0 with no complex above one element. `thinkers/peirce/probe_peirce_degeneracy.py` |
| 376 | thinkers/peirce H3 giving | computational | confirmed | Peirce (CP 1.345, 8.331): "A gives B to C" is genuine; "A's putting B away … C's subsequently taking B up" is "one dyadic relation followed by another." Genuine (G'=¬R, T'=¬R, R'=G∧T): adicity 3 (R←{G,T} φ=2.0), Φ=2.0 {G,T,R}. Degenerate (G'=¬R, T'=G, R'=T): adicity 2, yet ALSO Φ=2.0 {G,T,R}. Same whole, different relations: the imitation is a whole by return alone — Peirce's "thirdness externally appended" given a location. `thinkers/peirce/probe_peirce_giving.py` |
| 377 | thinkers/peirce H4 the sign relation | computational | partial | Peirce (CP 2.274, 5.484): sign, object, interpretant are "bound together," "not resolvable into actions between pairs." Exogenous dynamical object (O'=O, S'=O, I'=S↔O): adicity 3 ({O,S}→{S,I} φ=1.0, effect side; cause side 2) but Φ=0, core {O} alone — a genuine triad in a system that factors (probe 5: the source is out). Pragmatic feedback (O'=I): adicity 3 on the cause side (I←{O,S} φ=0.5), Φ=2.0 {O,S,I}. `thinkers/peirce/probe_peirce_sign.py` |
| 378 | thinkers/peirce H5 polyadic reducibility | computational | confirmed | Peirce (CP 1.363): tetrads are compounds of triads ("A sells C to B for the price D"). 60 random 4-node forms, each node reading 2 others via {AND,OR,XOR,NAND,NOR,XNOR} (seed 0): adicity {3: 2, 4: 58}; cause-side 4 in 43 (a pair jointly determined by a pair, e.g. {B,D}←{A,C} φ=1.0 in A=AND(C,D),B=OR(C,D),C=NOR(B,D),D=XNOR(A,C)); Φ>0 in 46, 4-member core in 7, Φ=3.0 in 4. Triads compose upward; with 374, dyads do not compose into joint determination. `thinkers/peirce/probe_peirce_polyads.py` |
| 379 | thinkers/caplow H1 power predicts the coalition | computational | partial | Caplow (1956, 1959): eight power types, a predicted pair for each. Rendered by his own assumptions (stronger partner controls the bloc; strength additive in the outcome; members read the outcome; 4 nodes A,B,C,O). A coalition BINDS iff both partners in the major complex. Binding = predicted in Types 1 (all three), 2 (BC), 4 (none), 6 (none); differs in 3 (BC binds, predicted AB/AC are {B,O},{C,O}), 5 (none binds; AB,AC→{A,O}, BC→{B,O}), 7, 8 (none; AB,AC→{A,O}). 4/8. Precoalition (weighted majority, no bloc) has NO complex in types 1,2,3,5,7,8 and {A,O} in 4,6. Whole Φ=0 for all 32 forms (a spectator always). `thinkers/caplow/probe_caplow_types.py` |
| 380 | thinkers/caplow H2 strength is weakness | computational | confirmed | Over the 5 binding coalitions (t1_AB, t1_AC, t1_BC, t2_BC, t3_BC) A is a party to 2, B to 4, C to 4 — strength is weakness among wholes. Over all 24 coalition forms A is in the core 18 times, B 7, C 6 — strength is strength among cores (dominance dyads {A,O}). `thinkers/caplow/probe_caplow_weakness.py` |
| 381 | thinkers/caplow H3 A ≥ B+C | computational | partial | Caplow: "true coalition is impossible" (Types 4, 6); at A = B+C (7, 8) BC "can block ... not sufficient to control." BC never binds in 4,6,7,8; Types 4,6: every form incl. precoalition core {A,O}. ADDITION: Type 7 (3,2,1) blocking BC (bloc = B) yields core {A,B,O} Φ=2.0 — the block binds A to his blocker; Type 8 (2,1,1) BC (bloc B∧C) core {A,C,O} 0.158 (B,C symmetric). Precoalition of 7, 8: no complex, so A not in every core. `thinkers/caplow/probe_caplow_dominance.py` |
| 382 | thinkers/caplow H4 conservative/revolutionary/improper | computational | partial | Caplow 1968, Type 5 (4,3,2): AB conservative → core {A,O}; AC improper → {A,O}; BC revolutionary → {B,O}. A out of the core only under BC (as the kinds imply); NO Type 5 coalition binds both partners — the junior partner is a spectator in all three. `thinkers/caplow/probe_caplow_kinds.py` |
| 383 | thinkers/caplow H5 strength is additive | computational | refuted | Caplow A.3: coalition strength = sum. 20 of 24 coalitions win; 5 bind; binding coincides EXACTLY with equal partner weights (equal winners 5/5, unequal winners 0/15). Same margin, different structure: t2_BC (4 v 3) binds, t3_AB (3 v 2) is {B,O}. Strength adds; wholeness does not (finding 3; probe 373). `thinkers/caplow/probe_caplow_additivity.py` |
| 384 | thinkers/heider H1 balance as sign product | computational | partial | Heider (1946: 110) / Cartwright–Harary: balanced iff sign product +. Rule: signed majority with hold, s_i' = maj(σ_ij·s_j, σ_ik·s_k, s_i), self-dual so switching-equivalent patterns are one structure. All 8 sign vectors → exactly 2 classes by (Φ, core, attractors), matching the product. BUT balanced class (+++, +−−) Φ=0.000, no complex; unbalanced (++−, −−−) Φ=6.000, core {p,o,q}. Heider's "unit" is the class that factors. `thinkers/heider/probe_heider_balance.py` |
| 385 | thinkers/heider H2 rest and tension | computational | confirmed | Balanced patterns: 2 rest states (all relations satisfied) each, all attractors rest. Unbalanced: no rest state; attractors = 6 frozen unsatisfied fixed points ("if a change is not possible ... tension") + 1 two-cycle ("forces ... will arise") each. `thinkers/heider/probe_heider_tension.py` |
| 386 | thinkers/heider H3 the all-negative triad | computational | refuted | Heider: −−− "too indetermined," a third kind; Davis 1967: clusterable. −−− and ++− identical: Φ 6.000, core 3, 7 attractors (6 fixed + 1 cycle). One person's complement carries one into the other. Two kinds, not three. `thinkers/heider/probe_heider_allnegative.py` |
| 387 | thinkers/heider H4 attitudes and relations coevolve | computational | confirmed | Six nodes (p,o,q,Lpo,Lpq,Loq); L_ij' = [s_i = s_j]. Whole Φ_MIP=0.000; core {p, o, Lpo} Φ=3.000 — two persons and their relation bind, the triad does not. 8 fixed points, ALL balanced rest states (2 unanimous +++, 6 two-against-one +−−); 12 two-cycles (attitudes/relations out of phase; synchronous-update caveat). ~2–4 min. `thinkers/heider/probe_heider_coevolution.py` |
| 388 | thinkers/heider H5 degree of balance (K4) | computational | refuted | Cartwright–Harary b(G) = positive cycles / 7 over the 8 switching classes of signed K4 (majority with hold over 3 signed inputs + self). Balanced class Φ=0.000 (core a dyad, 2.0); six single-negative-edge classes Φ=0.000 (core one node); the perfect-matching class (every person has one enemy) Φ=12.000, core all four. b(G) = 3/7 for BOTH the single-edge and the matching classes — blind where Φ sees. Φ tracks whether every member is torn, not the share of frustrated cycles. `thinkers/heider/probe_heider_degree.py` |
| 389 | thinkers/latour H1 the intermediary counts for one | computational | refuted | Latour (2005: 39): an intermediary "transports without transformation ... a black box counting for one." k = 0..3 copy-relays in a mutual dyad's tie: Φ_MIP = 2.000 at EVERY k (counts for one in Φ), but every relay is in the core — {A,B}, {A,I1,B}, …, {A,I1,I2,I3,B}. An intermediary in a loop counts for one more. `thinkers/latour/probe_latour_intermediary.py` |
| 390 | thinkers/latour H2 the mediator counts | computational | partial | Between A and B (A'=B, B'=M): copy M → core {A,M,B}; joint M'=A∧B → core {M,B} (A, a delayed copy of B, is evicted); specific M'=A⊕M → core {A,M,B}. All Φ 2.000. Mediators count; so does the intermediary; the joint mediator makes a neighbour count for nothing. `thinkers/latour/probe_latour_mediator.py` |
| 391 | thinkers/latour H3 actor = whatever makes a difference | computational | refuted | Latour (2005: 71). Boolean influence vs membership. Source (S'=S, M'=S∧A, A'=M): S makes a difference to M at every step, whole Φ_MIP 0.000, core {M,A} — S is outside what it drives. Spectator out too. Every core member makes a difference (necessary); not sufficient. Finding 3 restated: be read AND return. `thinkers/latour/probe_latour_difference.py` |
| 392 | thinkers/latour H4 the citizen-gun | computational | partial | Latour (1994: 31–34). Translation X'=C∧G, C'=X, G'=X → {C,G,X} Φ 2.000. Autonomous destiny G'=G, X'=G, C'=X → core {G} alone, Φ 1.000 (citizen and act out). Neutral tool G'=C, X'=G, C'=X → {C,G,X} Φ 2.000 — the "mere tool" is a relay in a loop and binds; identical to translation. The two myths are not symmetrical on this criterion. `thinkers/latour/probe_latour_gun.py` |
| 393 | thinkers/latour H5 the star of mediators | computational | confirmed | Latour (2005: 217). Center A'=M1∧M2∧M3. Mediators M_i'=A∧M_{i+1}: core all four, Φ 5.000. Intermediaries M_i'=A: core all four, Φ 3.000. Attachments first in both; the mediator adds integration, the intermediary adds a member. Latour's line sorts Φ, not membership. `thinkers/latour/probe_latour_star.py` |
| 394 | thinkers/burt H1 closure by cohesion, both brokers | computational | partial | Burt (1992: 18, 30): the hole is "an insulator"; the same holes give information and control benefits. Information broker E=X∧Y, control broker E=X∨Y, contacts or(E[,other]). Open: both in core Φ 2.000, V(E)=2.000, advantage 2.000. Symmetric tie X–Y: info broker EVICTED (core {X,Y}, whole 0.830 excluded); control broker stays as a peer in an OR-clique Φ 6.000, V(E)=4.000 = every member's, advantage 0. Closure removes the advantage of both and the membership of one. q214 one-sided replace bypass: both intrinsic, margin 0. `thinkers/burt/probe_burt_closure.py` |
| 395 | thinkers/burt H2 structural equivalence | computational | refuted | Burt (1992: 19): same contacts = redundant "regardless of the relation between" them. equivalent (X,Y both read Z): whole Φ_MIP 1.000, core {E,Y,Z} — one evicted, as Burt says. nonredundant (Z1,Z2): whole irreducible at 2.000 but major complex {E,Y,Z2} at 2.000 — a TIE; both contacts not in core → fails the rule. V(E)=2.000 with one, two equivalent, or two nonredundant contacts: a second contact across a hole adds nothing to what the broker adds (this topology). `thinkers/burt/probe_burt_equivalence.py` |
| 396 | thinkers/burt H3 constraint gradient | computational | partial | Burt (1992: 54–55): advantage falls with aggregate constraint. Three contacts, 0/1/2/3 ties: C_E 0.333/0.611/0.840/0.926; V(E) 3.000/0/0/0. Monotone, not graded: the first tie among contacts makes them a complex that excludes the broker (k1 whole Φ_MIP 0; k3 contacts an OR-clique 6.000, E's positional advantage −4.000). The hole is a cliff, not a slope. `thinkers/burt/probe_burt_constraint.py` |
| 397 | thinkers/burt H4 the rival broker | computational | confirmed | Burt (1992: 45): holes around your contacts "and none attached to yourself." Rival R=X∧Y beside E=X∧Y, contacts or(E,R): whole reducible (Φ_MIP 0), largest complex {R,Y} 0.189, V(E)=V(R)=−1.811 (without either the rest is the conjunctive triad at 2.000). A replacement does not divide the advantage; it removes the whole. `thinkers/burt/probe_burt_rival.py` |
| 398 | thinkers/burt H5 closure within, brokerage beyond | computational | refuted | Burt (2000/2001/2005): closure inside the group realizes the value of brokerage beyond it. E=A1∧A2∧O: open V(E)=3.000 (advantage 2.000); within (A1–A2) 0; across (A1–O) 0; everywhere 0 (contacts a clique 6.000, E −4.000). Any closure around the broker closes over him. Burt 1992 beats Burt 2001. `thinkers/burt/probe_burt_complement.py` |
| 399 | thinkers/granovetter H1 strength is graded | computational | confirmed | Granovetter (1973: 1361): strength a graded symmetric combination, first dimension time. Tie of strength p = each endpoint reads the other w.p. p per step; AND of reads, hold if none; exact Φ on the stochastic TPM. Dyad Φ 0.252 / 0.658 / 1.236 / 2.000 at p = .25 / .5 / .75 / 1 — monotone, not linear. `thinkers/granovetter/probe_granovetter_strength.py` |
| 400 | thinkers/granovetter H2 the forbidden triad | computational | confirmed | Granovetter (1973: 1363): with A–B, A–C strong the B–C tie is "always present (whether weak or strong)." Open = the conjunctive triad Φ 2.000; weak-closed 2.830; strong-closed 6.000; core {A,B,C} each. The forbidden triad is already a whole; each closure adds integration in his order. `thinkers/granovetter/probe_granovetter_triad.py` |
| 401 | thinkers/granovetter H3 the bridge | computational | refuted | Granovetter (1973: 1364): "all bridges are weak ties"; a bridge is the only path. Two strong dyads + bridge a2–b1: transmission 12 pairs weak or strong; major complex ONE DYAD {b1,b2} 2.000 under both; whole Φ_MIP 0.830 (weak), 2.000 (strong — ties the dyad). A bridge of any strength transmits across and integrates nothing across. `thinkers/granovetter/probe_granovetter_bridge.py` |
| 402 | thinkers/granovetter H4 damage | computational | partial | Granovetter (1973: 1366): removing a weak tie damages transmission more. Cut the weak bridge: −8 pairs; cut a strong within-dyad tie: −6. Major-complex Φ: unchanged either way (surviving dyad 2.000); whole Φ_MIP 0.830 → 0 either way. Right on transmission; nothing to lose on integration. `thinkers/granovetter/probe_granovetter_damage.py` |
| 403 | thinkers/granovetter H5 local cohesion, overall fragmentation | computational | partial | Granovetter (1973: 1373–78): cliques without weak ties are fragmented at the aerial view; weak ties integrate. Three strong dyads: isolated Φ_MIP 0; weak CHAIN transmission 30 but core still a dyad (whole 0.830); weak RING core all six at 2.490; strong ring 4.000. Weak ties integrate when they close a cycle, not when they form a path. ~6 min. `thinkers/granovetter/probe_granovetter_community.py` |
| 404 | thinkers/bowen H1 the smallest stable system | computational | refuted | Bowen (1978: 373): the triangle is "the smallest stable relationship system." Togetherness = reading, anxiety = flip noise, stability = Φ retained. Triangled form (A reads B∧C; B reads A; C reads A∧B) retains LESS than the dyad at every ε (.794/.836, .618/.687, .347/.434, .167/.238) and its core is {A,C}, never the triad. Post hoc: the reactive triangle (both insiders involve C) is a triad at every ε, above the dyad in absolute Φ, below in fraction. `thinkers/bowen/probe_bowen_stable.py` |
| 405 | thinkers/bowen H2 twosome and outsider | computational | refuted | Bowen (1978: 373, 174): calm = close twosome + outsider; tension "triangles" the third. Calm core {A,B}, C out ✓. Triangled core {A,C} at 2.000 (whole 1.000): the third DISPLACES the other insider rather than joining — Bowen's own rotation ("one of the original insiders now becomes the new outsider"), not his unit claim. `thinkers/bowen/probe_bowen_twosome.py` |
| 406 | thinkers/bowen H3 the outside position costs least | computational | confirmed | Bowen (1978: 373–74): "in periods of stress, the outside position is the most comfortable." Anxiety at one position of the triangled form: loss at ε=.1 A .237, B .237, C .100; at .2 .458/.458/.200. Anxiety at B (outside the {A,C} core) costs the core nothing; anxiety at C moves the core back to {A,B}. The anxious party is the one left out. `thinkers/bowen/probe_bowen_outside.py` |
| 407 | thinkers/bowen H4 interlocking triangles | computational | refuted | Bowen (1978: 373–74): a fourth is triangled by the same move; the system is "a series of interlocking triangles." At ε=0 the four are ONE complex {A,B,C,D} at 2.000 (pre-registered test fails); at every ε>0 the core is the triangle {A,C,D}; the four retain less than the three (.728/.794 … .097/.167). The triangle is the unit of the anxious system, not the calm one. `thinkers/bowen/probe_bowen_interlock.py` |
| 408 | thinkers/bowen H5 detriangling | computational | partial | Bowen (1978: 174–75): a third in contact with both who does not react lets the twosome's tension resolve. Neutral C (reads both, read by both, holds) is OUT of the core; reactive C is IN (clique 6.000) ✓. But the insiders' core retains no more with the neutral third than the bare dyad (.836/.836, .686/.687, .423/.434, .210/.238) ✗ — under anxiety a read-but-unresponsive party is a noise source. `thinkers/bowen/probe_bowen_detriangle.py` |
| 409 | thinkers/hegel H1 the middle founds the connection | computational | refuted | Hegel (SL 588; EL §181R): "everything is a syllogism." Syllogism = the conjunctive triad under Hegel's labels: Φ 2.000, core {A,M,B}. Judgment (copy dyad) also 2.000 — the middle adds a member, no integration. Minus-M: whole Φ 0, but a self-holding extreme counts as a one-node complex (Φ 1) so the pre-registered "no complex" clause failed (post hoc with constants: no complex). `thinkers/hegel/probe_hegel_middle.py` |
| 410 | thinkers/hegel H2 the external middle | computational | confirmed | Hegel (EL §182; SL 635–36): the formal middle is "only the abstract particularity," communication passes "without transition into the opposite." Relay (A holds, M copies A, B copies M) and broadcast: Φ 0, every term reachable from A, M in no complex. The conduit result in Hegel's vocabulary. `thinkers/hegel/probe_hegel_external.py` |
| 411 | thinkers/hegel H3 return through the middle | computational | confirmed | Hegel (EL §182): "the subject joins itself together with itself by means of this mediation." Half-return (M reads B, B does not read M): core {A,M}, B out; syllogism core {A,M,B}. Return is the membership condition. `thinkers/hegel/probe_hegel_return.py` |
| 412 | thinkers/hegel H4 the system of three syllogisms | computational | partial | Hegel (SL 642–43; EL §198R): each term "runs through the determination of the middle term and the extreme"; "the state is a system of three syllogisms." Shares (deletion cost): syllogism A 0, M 1.000 (2.000 with orphans constant), B 0 — the middle bears all; system (clique) 4.000 each — no binder. Φ(system) 6.000 = 3 × Φ(syllogism). `thinkers/hegel/probe_hegel_system.py` |
| 413 | thinkers/hegel H5 the rotating middle | computational | partial | Moss's (2013) process reading: each term middle in turn by a three-phase clock. Rotating core {A,B,C} at 2.000 with the clock out — ONE syllogism's Φ, not the system's 6.000. Alternation in time spreads the middle's role and does not sum the syllogisms; simultaneity does. `thinkers/hegel/probe_hegel_rotating.py` |
| 414 | thinkers/girard H1 the straight line is not essential | computational | refuted | Girard (1965: 2): "the mediator is there, above that line, radiating toward both." S copies M; M = S∧O; O = M∧S. Internal core is {M, O} at 2.000 — the SUBJECT is out (whole 1.000). Minus M: no complex ✓. Subject and object are not one through the mediator; the mediator and object are one without the subject. `thinkers/girard/probe_girard_triangle.py` |
| 415 | thinkers/girard H2 external and internal mediation | computational | refuted | Girard (1965: 9): external = no contact between spheres; internal = penetration. External: S, O not one ✓ (M alone a self-holding one-node complex, clause failed on the artifact). Internal core {M, O}, not the triad. The distinction survives — no complex spans the spheres when external, one does when internal — but the complex is a pair. `thinkers/girard/probe_girard_mediation.py` |
| 416 | thinkers/girard H3 the object is a means of reaching the mediator | computational | confirmed | Girard (1965: 10, 99): "the impulse toward the object is ultimately an impulse toward the mediator"; each copies "the copy of his own desire." Double mediation core {S, M} 2.000 = bare dyad 2.000; O out. The object adds nothing. `thinkers/girard/probe_girard_object.py` |
| 417 | thinkers/girard H4 distance | computational | confirmed | Girard (1965: 9): "we have only to vary the distance." M reads S w.p. p: core {M} alone at p ≤ .25; {S, M} at p = .5 (1.000), .75 (1.500), 1 (2.000). O in the core at no p. A threshold, then a gradient. `thinkers/girard/probe_girard_distance.py` |
| 418 | thinkers/girard H5 doubles | computational | confirmed | Girard (1965: 122): "differences ... gradually dissolve." Shares: double S 2.000, M 2.000, O 0 — the rivals interchangeable; internal S 0, M 2.000, O 0 — the disciple dispensable, the model not. `thinkers/girard/probe_girard_doubles.py` |
| 419 | thinkers/serres H1 the one-way arrow is the atom of relation | computational | refuted | Serres (1982: 7–8): "the simple, irreversible arrow ... the atomic form of our relations." A parasite reading a constant producer: no complex, Φ 0. Exchange A ⇄ B: 2.000. The lab's standing result; the instrument sides with Simmel. `thinkers/serres/probe_serres_arrow.py` |
| 420 | thinkers/serres H2 noise gives rise to a new system | computational | confirmed | Serres (1982: 14): "Theorem: noise gives rise to a new system, an order that is more complex than the simple chain." Chain F → T → R: no complex. Add N' = R, R' = T ∧ ¬N: complex {R, N} 2.000 at the last position; F and T out. The interruption is the chain's only return. `thinkers/serres/probe_serres_theorem.py` |
| 421 | thinkers/serres H3 the excluded third, included | computational | refuted | Noise from a coin XORed onto the A→B channel does not lower the pair's Φ — it removes the pair, at every ε ∈ {0, .05, .1, .25, .4, .5} (post hoc). PyPhi's cause side infers a background unit's past from the present; a coin's past leaves no trace, so B' = A ⊕ N carries no cause information about A. The system's own noise (N' = A ∧ B) is outside the complex; core {A, B} 2.000 unchanged. `thinkers/serres/probe_serres_third.py` |
| 422 | thinkers/serres H4 the last position wins | computational | refuted | Serres (1982: 4): "the last to come tries to supplant his predecessor." Cascade with every level ejecting the one above: core {T, R, N} 2.000 — all but the producer. Shares F 2.000 (outside the complex, indispensable), T 2.000, R 2.000, N 0 (inside, dispensable). Membership and indispensability come apart; the excluded-indispensable one is the producer. `thinkers/serres/probe_serres_cascade.py` |
| 423 | thinkers/serres H5 the quasi-object weaves the "we" | computational | confirmed | Serres (1982: 225): "when being passed, makes the collective, if it stops, it makes the individual." Token ring A' = C, B' = A, C' = B: {A, B, C} 2.000. Token frozen (constants): no complex. `thinkers/serres/probe_serres_quasiobject.py` |
| 424 | thinkers/davis H1 Theorem 1 on the four triangles | computational | confirmed | Davis (1967: 181): clusterable iff no cycle with exactly one negative line. Persons hold a camp from k ∈ {2,3,4} (two bits), least-strain update. k ≥ 3: ppp, pmm, mmm rest; ppm never; k = 2: mmm has no rest state. `thinkers/davis/probe_davis_theorem.py` |
| 425 | thinkers/davis H2 the classes follow clusterability | computational | confirmed | At k = 4: ppm core {p, o, q} Φ 6.000 (the Heider number, on the high bits); ppp no complex; pmm core {p, o} 2.000; mmm core {p, q} 2.000. One pattern apart, three together — Davis's classes, not Cartwright–Harary's. ~18 min. `thinkers/davis/probe_davis_classes.py` |
| 426 | thinkers/davis H3 the all-negative verdict depends on the alphabet | computational | partial | mmm core persons at k = 2, 3, 4: 3, 2, 2 — the Heider H3 refutation (−−− ≡ ++−) was the two-camp verdict and is withdrawn. ppm: 3, 2, 3 — the k = 3 encoding (dead code) dips; at full alphabets (one bit; k = 4) ppm is a whole of three at 6.000 both times. ~18 min. `thinkers/davis/probe_davis_alphabet.py` |
| 427 | thinkers/davis H4 Theorem 1 on K4, triangles decide, unique clustering | computational | confirmed | 64 sign patterns at k = 4: 15 clusterable (8 balanced); rest states exist iff clusterable 64/64; triangle test = cycle test 64/64; one partition per clusterable pattern 15/15. Synchronous swap cycles in 11/15. `thinkers/davis/probe_davis_census.py` |
| 428 | thinkers/davis H5 the incomplete graph | computational | confirmed | Negative path p–o–q, no pq, k = 4: two partitions at rest ({p}{o}{q}, {p,q}{o}); Φ_MIP 0; core {o, q} 2.000 — one negative pair, not the mediator with both. `thinkers/davis/probe_davis_path.py` |
| 429 | thinkers/coleman H1 closure makes the norm effective | computational | confirmed | Coleman (1988: S105–S106): open, B and C "cannot combine forces to sanction A"; closed, they "combine to provide a collective sanction." A' = B ∧ C; B' = ¬A [∨ C]. Open: no fixed point (sanction cycles), core {A, B, C} 2.000. Closed: 111 fixed, core {B, C} 0.369, A outside. The norm holds and the whole loosens. `thinkers/coleman/probe_coleman_norm.py` |
| 430 | thinkers/coleman H2 closure is the collective's gain, not the broker's | computational | confirmed | Burt's own forms re-read for the whole: info coreΦ 2→2, E's advantage 2→0; control coreΦ 2→6, advantage 2→0. The Burt H1 partial is Coleman's confirmation. `thinkers/coleman/probe_coleman_broker.py` |
| 431 | thinkers/coleman H3 intergenerational closure makes the four one | computational | refuted | Parents' tie: core moves from {P2, K2} to {P1, P2} at 2.000; children's value added 0. A coalition forms above the parties it governs. Basin of 1111 is 1/16 in both forms. `thinkers/coleman/probe_coleman_parents.py` |
| 432 | thinkers/coleman H4 the rotating-credit association is one unit | computational | confirmed | Coleman (1988: S102–S103). Ring of four: core all four 2.000, every V 2.000 (any absconder destroys it). Two bilateral credits: core one pair. `thinkers/coleman/probe_coleman_credit.py` |
| 433 | thinkers/coleman H5 the public good | computational | refuted | Coleman (1988: S119): the creator "captures only a small part." Closing the norm form: core Φ 2.000 → 0.369 (gain −1.631); ΔV(B) −1.631, fraction 1.000. No gain to capture; closure levels every party to 0.369. `thinkers/coleman/probe_coleman_public.py` |
| 434 | thinkers/ostrom H1 the monitored are in the structure | computational | confirmed | Ostrom (1990: 90, 95): monitors "are the appropriators." Each of three complies if the others do or if jointly sanctioned, and sanctions when either other defects. Core {c1, c2, c3} 2.862, whole 0.830, sanction bits outside adding 0.862 each. Coleman's closed norm beside it: core {B, C}, A out. `thinkers/ostrom/probe_ostrom_monitors.py` |
| 435 | thinkers/ostrom H2 self-governance against Leviathan and privatization | computational | partial | Clique of contingent commitments 6.000 (all three in) vs enforcer's star 3.000 (all four in, all V 3.000, enforcer's advantage 0). Privatization: self-holding singletons count as complexes of one. `thinkers/ostrom/probe_ostrom_solutions.py` |
| 436 | thinkers/ostrom H3 nested enterprises are one whole | computational | refuted | Two cliques under a federation: whole of seven irreducible at 2.000, exclusion returns one clique at 6.000; F outside the core with V 6.000, delegates 4.000, rank and file 0. Both cliques irreducible at 6.000 at the all-on state (post hoc). The Coleman parents pattern one level up. `thinkers/ostrom/probe_ostrom_nested.py` |
| 437 | thinkers/ostrom H4 quasi-voluntary compliance | computational | confirmed | Ostrom (1990: 94): sanctions "surprisingly low." mutual's only fixed point is 111000 (comply, no sanction); each single lapse returns in 4 steps. Coleman's closed norm rests at 111 (sanction standing). `thinkers/ostrom/probe_ostrom_compliance.py` |
| 438 | thinkers/ostrom H5 boundaries | computational | partial | An unread outsider changes nothing (core and Φ as self, V(O) 0). A breach (a member reads the outsider) leaves core and Φ unchanged and makes the outsider indispensable, V(O) 6.000 — outside and necessary. `thinkers/ostrom/probe_ostrom_boundary.py` |
| 439 | thinkers/blau H1 contingent service makes power a relation | computational | confirmed | Blau (1964: 22): power by "making the satisfaction of their need contingent on their compliance." Constant supplier + dependent: no complex (the Serres arrow). Contingent supplier: {S, B} 2.000. `thinkers/blau/probe_blau_contingent.py` |
| 440 | thinkers/blau H2 an alternative supplier removes the superior's power | computational | confirmed | Blau (1964: 118), second alternative. Single: V(S) 2.000, advantage 0. Two suppliers: V(S1) 0, advantage −2.000, V(B) 2.000 — Burt's control broker from the contacts' side. `thinkers/blau/probe_blau_alternatives.py` |
| 441 | thinkers/blau H3 organizing the dependent shifts power | computational | confirmed | Subordinates who comply only together: core moves from {S, B1, B2} to {B1, B2} at 2.000; S's advantage 2.000 → 0; S outside the core with V 2.000 (outside, indispensable). `thinkers/blau/probe_blau_organized.py` |
| 442 | thinkers/blau H4 legitimate authority is carried by the subordinates | computational | confirmed | Blau (1964: 23): consensus becomes "group pressures that promote compliance." S off, both complying: power 011 → 100, authority 011 → 111. Authority core {B1, B2} 2.000, V(S) 0 (outside, dispensable). `thinkers/blau/probe_blau_authority.py` |
| 443 | thinkers/blau H5 opposition mirrors legitimation | computational | partial | Shared opposition rests at S yielding; isolated never rests (Coleman's cycle). But the approving pair binds at 2.000 and the opposing pair at 0.369: same shape, not the same structure. `thinkers/blau/probe_blau_opposition.py` |
| 444 | thinkers/emerson H1 the C–A–B network is unbalanced with A indispensable | computational | confirmed | Emerson (1962: 32, 37). A' = B ∨ C; B' = A; C' = A. Core all three 2.000; V(A) 2.000, V(B) = V(C) 0; A's advantage 2.000. `thinkers/emerson/probe_emerson_network.py` |
| 445 | thinkers/emerson H2 withdrawal balances (operation 1) | computational | confirmed | B holds its own state: core {A, C} 2.000, B out, V(A) 2.000 → 1.000, V(C) 0 → 1.000, advantage 0. The only operation that takes from A. `thinkers/emerson/probe_emerson_withdrawal.py` |
| 446 | thinkers/emerson H3 extension balances and keeps three actors (operation 2) | computational | confirmed | B–C tie: OR-clique 6.000, all in, every V 4.000, advantage 0 — Emerson's "still just a power network"; Burt's closed control broker. `thinkers/emerson/probe_emerson_extension.py` |
| 447 | thinkers/emerson H4 status-giving balances A–B only (operation 3) | computational | confirmed | A' = B: core {A, B} 2.000, V(A) = V(B) 2.000, C outside with V 0. `thinkers/emerson/probe_emerson_status.py` |
| 448 | thinkers/emerson H5 extension reduces the strong, coalition raises the weak | computational | partial | Both zero A's advantage; coalition (AND-clique) raises V(B), V(C) 0 → 6.000; extension raises V(A) 2.000 → 4.000 rather than lowering it. Network vs collective actor = a member adds 4.000 (a pair survives deletion) vs 6.000 (nothing does). `thinkers/emerson/probe_emerson_coalition.py` |
| 449 | thinkers/peirce JD1 the pre-registered joint-determination criterion reads the registered exemplars | computational | confirmed | Criterion fixed in c7535bc: a one-element mechanism with an irreducible cause purview of two other elements, in the major complex of a Φ > 0 whole. Control M ← AB, giving R ← GT, pragmatic sign I ← OS in a whole; seven negatives none; misreads 0. `thinkers/peirce/probe_peirce_joint_determination.py` |
| 450 | thinkers/peirce JD2 Simmel's majority: a party jointly determined in a system that factors | computational | confirmed (tie-dependent, post hoc) | majority_triad Φ_MIP = 0, C ← AB φ 0.5, but AB/AC/BC tie at 0.5; triad_mutual 6.0 and mediator 2.0 in a whole. `thinkers/peirce/probe_peirce_joint_determination.py` |
| 451 | thinkers/peirce JD3 confirmation on unread families | computational | confirmed | n = 5 one-input wirings, 13 isomorphism classes: 0 joint determination. Seed-1 two-input sample, 30 forms: 5 in a whole, 11 with Φ_MIP = 0 (all tie-robust), 1 none. `thinkers/peirce/probe_peirce_joint_determination.py --full` (slow) |
