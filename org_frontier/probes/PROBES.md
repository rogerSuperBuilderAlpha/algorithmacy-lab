...
| 279 | Major-complex spectator-robustness (H1) [dry-run test] | A fully decoupled spectator node does not enter the core or move Φ | **confirmed** | Adding a decoupled spectator X (X'=X) to the canonical strict-mediation triad (W'=S, S'=W∧C, C'=S) leaves the major complex exactly {W,S,C} at Φ=2.0 — the spectator stays out, the core is unchanged, and Φ is unchanged. Makes the spectator-robust reading the verdict relies on an explicit, registered control. `probe_spectator_robustness.py` |
