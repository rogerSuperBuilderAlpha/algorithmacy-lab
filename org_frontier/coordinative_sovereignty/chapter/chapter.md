...
sovereignty is the standing to shape the terms on which one is mediated, to contest the mediator's
arbitrary power, and to hold a share in the governance of an arrangement one cannot leave. A counterfactual
sorts mediators an actor can leave from those it can only govern and decides which institutions to build.
...
and the NIS2 Directive mandates cybersecurity governance. Together these instruments give European actors a
...
customers, on an application store whose ranking and review decisions it can neither usefully see nor
predict nor contest. Disclosure duties and complaint channels now reach parts of that layer, and section
3.2 takes them up. They still leave the merchant whose listings disappear, and the creator whose audience
vanishes in a recommendation change announced nowhere, without standing over the determination that decided
the outcome. Compliance governs how these actors handle data. Their market positions are decided somewhere
else, inside the algorithmic systems through which the platform chooses what to surface and what to bury
...
Platform coordination introduces a relationship the pattern does not cover. A worker, a seller, or a
...
it. What sets the situation apart can be put as a question about the coordination's rules. Restore the
...
leverage on a bypass that does not exist, and it redefines freedom as the absence of the coordination
...
sovereignty asks what autonomy remains for the actor coordinating through it. The two vary independently,
and the rest of the chapter tells them apart.
...
term. The democratic tradition dissolved that difficulty long ago. Habermas (1996) describes popular
sovereignty as desubstantialized, no longer seated in a person or an assembly at all: "sovereignty is found
in those subjectless forms of communication" (pp. 486–487). Sovereignty in that sense is the standing to
hold power to account through the medium in which power is exercised, and it is the sense a literate public
exercises over a state whose writing it can read. Coordinative sovereignty adds two attributes to it, a
mediation the actor cannot exit and a standing carried by voice, and so narrows the term's extension.
...
innovate the concept of sovereignty as a whole" (Abstract).

## 2. Sovereignty and its sensibilities

Habermas established that a form of authority and a communicative competence arrive together. The question
he did not press, and the one platform coordination forces, is who holds the competence. A competence that
everyone has governs an authority everyone can contest. A competence held by a few governs an authority the
rest merely endure. Under a mediation an actor cannot leave, that distribution decides everything, because
an actor who cannot leave and cannot contest has nothing left.
...
forms of authority with the media that carry them, from Innis (1951) on how a civilization's dominant
medium biases its form of power to Eisenstein (1979) on the printing press and the authority of church and
state. One modest claim survives every qualification of that tradition: a sovereignty and its competence
form together, and no sovereignty can be contested by those who lack the competence its medium requires.
...
a new situation with the competence they already have. Faced with platform coordination, the literate
...
between (Obstfeld, 2005). The platform occupies the mediating position that J. D. Thompson (1967) named a
...
things. His informal co-optation absorbed outside actors into an organization's leadership and gave them
real influence in exchange for their compliance. His formal co-optation did not: it "does not envision the
transfer of actual power. The forms of participation are emphasized but action is channeled so as to
fulfill the administrative functions while preserving the locus of significant decision in the hands of the
initiating group" (Selznick, 1949, pp. 13–14). Platform enrollment is formal co-optation with even the
forms of participation removed. The seller has no seat and no ceremony of consultation, only a
determination. The term is narrowed along Selznick's own axis.
...
directly and divide the surplus of their exchange between them. In the mediated triad both parties must
pass through the third party to produce anything at all, and that structural position gives the mediator
...
buyers. She messages first at the smallest sign of trouble, absorbs returns she could contest, and prices
the difference in. She has read nothing. There was nothing to read. She has inferred what the system
rewards from the movement of her own listings, rendered her conduct toward a counterpart into the terms she
believes the system reads, and she will do it again in six months when the weighting changes. A seller
without the competence sees only that sales fell, and waits, or blames the market, or abandons a channel
that was working.
...
the actors best placed to model the system are the large ones already advantaged within it. Survey evidence
bears this out: algorithmic knowledge varies with socioeconomic advantage instead of spreading evenly
across those subject to it (Cotter & Reisdorf, 2020). Petre et al. (2019) identify a mechanism that holds
the gradient in place. Platforms treat attempts to work the system as illegitimate "gaming," and the
accusation falls hardest on producers who are not already established, so the actors with least room to
absorb a penalty are the ones most exposed to one for acquiring the competence. The platform, for its part,
does more than neglect to make its rules legible, since opacity preserves the rent (section 3.3).
Coordinative sovereignty, which algorithmacy makes possible, therefore concentrates where algorithmacy is
already held.
...
Which sovereignty is available to a coordinated actor depends on a property of the coordination's rules,
and the property can be checked. Two questions do the work, and they are not the same question.

### 5.1 When is the coordination genuinely triadic?

Ask first whether the three parties — a worker, a mediator, and a counterpart — really act as one system,
...
Act's anti-steering obligations run that edge operation, and what happened next is instructive.

An opened bypass is not yet an exit. Ordered in April 2025 to stop restricting how developers steer users
...
not produce voice. It forecloses exit and leaves voice as the residual lever, one available against
contingent mediators too and the only one left where the mediator is necessary. Whether a coordinated actor
...
Where the mediator is necessary and only voice remains, autonomy has to be reconceived. The received idea
of sovereignty as independence has nothing to offer an actor who cannot leave, and the digital extension of
...
networking, on mixed evidence of demand. That review, the statutory one due after the Act's first years,
found the regime fit for purpose and proposed no legislative amendment, so the obligations described here
are the stable part of a shifting landscape. A regulator sorting functions by whether interoperability
...
| Instrument | Value where exit is available | Value where exit is foreclosed | Who can invoke it |
|---|---|---|---|
...
| Antitrust and anti-steering remedies | opens the bypass, at whatever price the mediator can still charge for it | alters the terms of dependence rather than its degree | a regulator or a complainant; a dependent firm can use a remedy already granted |
| Contestability of individual decisions | generates the record an exit campaign runs on | voice at the level of the case | a natural person under Article 22; a business user through the P2B complaint and mediation route |
| Collective bargaining | builds the counterparty an exit campaign needs, where the bearer may lawfully bargain | voice at the level of the rule | workers and their representatives; firms only within the limits competition law sets on associations of undertakings |
...
| Oversight boards | little, since a bypassable determination is better contested elsewhere | voice, bounded by the constituting platform | the platform constitutes it; dependent parties appear before it |

The last column is the one a dependent party reads first, and it is unflattering. Of the seven, a small
firm can reach two on its own account, and one of those two is the complaint and mediation route the
Platform-to-Business Regulation supplies, which the simplification package would repeal. The rest require a
regulator, a collective, or capital. An instrument's value where exit is foreclosed is not the same as its
availability to the actor for whom exit is foreclosed, and a policy discussion that runs the middle column
without the last one prescribes remedies to parties who cannot invoke them.
...
mechanism confers standing in practice has not yet been reported in the research literature.
...
to a disclosure. In November 2025 the Commission proposed to repeal it as part of a wider simplification
package, which had not reached trilogue as of August 2026 (European Commission, 2025a); a separate omnibus
covering the AI Act was adopted earlier that year. The repeal would remove the duty owed to a business user
to explain the parameters by which its own goods and services are ranked for others. The Digital Services
Act's recommender transparency runs in the other direction, explaining to a recipient why particular
information is being surfaced to that recipient, and it does not replace the duty the Regulation owes to
the ranked party. High digital sovereignty beside low coordinative sovereignty, and an instrument that
built a little of the second now proposed for withdrawal.
...
2 December 2026 provide a rebuttable presumption of employment where the platform controls the work, along
with information rights over the automated systems, human review of consequential automated decisions, and
limits on the data the platform may process (European Parliament & Council of the European Union, 2024).
Transposition falls due on 2 December 2026, and the directive therefore describes standing that member
states are obliged to create rather than standing platform workers presently hold. How much of it survives
twenty-seven transpositions is the question, and the directive leaves the presumption's trigger — facts
indicating direction and control, as national law, collective agreements, and practice define them — to the
member states, which is where the variation will come from. Its Article 25 obliges member states to promote
...
Standing over the coordination is being built while control of the infrastructure stays at zero.
...
determinations that set its market outcomes. The exposure has two parts, and treating them as one wastes
the remedy. The first is a competence gap, which a firm can close on its own account by building
algorithmacy. The second is a standing gap, which no competence closes, because standing is conferred by
the terms of the coordination and by the institutions that hold those terms in place. The diagnostic gives
managers a decision procedure for the second and tells them what the first should be spent on. Where the
firm depends on a contingent mediator, the move is toward exit: support interoperability, build or back an
alternative, break the lock-in. Where it depends on a necessary mediator, exit is closed and the move is
toward voice: organize with other dependent firms, pool data, demand contestable systems, seek a standing
channel of governance. The Platform-to-Business Regulation supplies the clearest existing instance of such
a channel for a dependent firm, and its proposed repeal would close it, which is the practical stake of the
simplification package for the actors this section addresses. Where the dependence is partial, which is the
ordinary case, the two branches run at once against different functions, and the firm's task is to keep
them apart rather than to average them into a single posture. Treating a necessary mediator as contingent
wastes effort on an exit that cannot be had. Section 5.5 shows the alternative at sector scale: European
hotels met an aggregating function they could not leave by building the competence to work it, and met a
parity clause they could leave through contract law and competition enforcement. Treating a contingent
mediator as necessary concedes a bypass that could be opened.

Run across a firm's dependencies, the diagnostic yields a figure a board can act on: the share of revenue
that arrives through necessary functions against the share that arrives through contingent ones. The first
names a governance exposure that will persist and has to be managed; the second names a project with a
completion date. Firms that report platform dependence as a single percentage cannot tell the two apart.
...
can each be trained, budgeted, and audited the way compliance now is. None of it will show up on a
compliance audit.
...
The typology's contribution is conceptual, and the empirical program it opens begins with measurement. Five
...
in principle (Dogruel et al., 2022). An algorithmacy scale would have to show that it captures what those
do not: competence exercised on a counterpart and a system at once, where the system is scoring the actor
on how the coordination with the counterpart goes. Because self-reported competence invites
...
The fifth tests the typology's central claim rather than assuming it. Section 8 holds that digital
sovereignty and coordinative sovereignty vary independently across firms, and independence is an estimable
quantity. Administering an organizational digital-sovereignty measure and a coordinative-sovereignty
measure to the same firms returns a correlation, and the typology predicts a modest one: firms high on
compliance capacity and infrastructural control should be spread across the range of standing within the
coordinations they depend on. The two should also predict different criteria, regulatory-audit outcomes for
the first and recovery from an unannounced change in a ranking or a platform policy for the second. A high
correlation would collapse the typology into the single capability the field has assumed, which is the
outcome that would refute the chapter.
...
deployers to ensure a level of AI literacy among staff and the platform-work directive requires member
states to support workers' representatives with expertise the platform funds. Whether coordinative
sovereignty becomes general or stays the possession of those who can afford the competence turns on which
of these two paths carries the diffusion.
...
and the standing it makes possible is coordinative sovereignty: effective voice within a coordination one
cannot leave.
...
coordinate the rest, and the market that currently teaches it rewards compliance. Sovereignty in the age of
artificial intelligence rests on the standing of the coordinated to govern the coordinations that govern
them, and on the diffusion of the competence that standing requires. Neither reading alone nor control
alone secures it.
## References
