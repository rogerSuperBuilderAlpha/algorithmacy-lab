...
## Analytic dissociation (`python -m psi_vs_phi.dissociation`)
...
## Reproduce
`python -m psi_vs_phi.psi` (controls) → `python -m psi_vs_phi.run 15 1` (… seeds 1–5) →
`python -m psi_vs_phi.exact_phi3 15 1` (3.0, n=3) → `python -m psi_vs_phi.analyze` →
`python -m psi_vs_phi.figures`.
