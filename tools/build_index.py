...
def _submissions() -> list:
    """Outward manuscripts under submissions/."""
    base = os.path.join(_ROOT, "submissions")
    readme = os.path.join(base, "README.md")
    if not os.path.exists(readme):
        return []
    title = _first_heading(readme) or "Submissions"
    summary = _clip(_first_paragraph(readme))
    out = [f"- **[{title}]({_link(readme)})** — {summary}"]
    cal = os.path.join(base, "CALENDAR.md")
    ...
    for name in ("coordinative_sovereignty", "hospitality_phygital", "lima_pdw",
        ...
        if not os.path.exists(r):
            continue
        out.append(f"- [{_first_heading(r) or name}]({_link(r)}) — {_clip(_first_paragraph(r))}")
    return out
...
def _syntheses() -> list:
    ...
    for name, gloss in [
        ...
        ("PR739_PACKAGE.md", "PR #739 merge package — closed arcs, instrument, human gates"),
        ("OMIT_ATOM_ARC.md", "omit/discrete-Φ arc — motif-ruled atoms, scale morphs"),
        ("CONSTRUCT_LADDER_ARC.md", "construct×ladder arc — COMMIT_READ, Φ=n−1 flip, gate seal"),
        ("ESTIMATION_ARC.md", "estimation lane — topology bottleneck; closable"),
        ("STOCH_TEMPORAL_ARC.md", "stoch–temporal arc — noise, delay, coexistence, CT proxy; closable"),
        ("FORMAL_THEORY_ARC.md", "formal theory arc — #47–#50; closable"),
        ("AI_MULTIAGENT_ARC.md", "AI/multi-agent arc — membership cuts; closable"),
        ("POLITICAL_ECONOMY_ARC.md", "political economy arc — #29–#36; closable"),
        ("BEYOND_BINARY_ARC.md", "beyond-binary arc — mv pin M1/M2; #1–#4; closable"),
...
def build_directory() -> str:
    ...
    if submissions:
        L += ["### Submissions — outward manuscripts", "",
